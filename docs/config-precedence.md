...
A higher layer always wins. Layer **N** overriding layer **N+1** is the whole
point of having N+1 layers instead of one — a CLI flag beats an env var beats
...
Layers 3–6 are all files (or, for layer 6, hardcoded fallback lines) read by
...
pre-existing value (from layer 2's env export, or from a higher layer sourced
...
highest-layer-first, the ladder falls out for free — no special-casing, no
"which value wins" logic anywhere. The one rule that makes it work: **every
layer's file must use `:=`, with no exceptions.** A single plain assignment
anywhere in the chain breaks the property for every layer below it, because a
...
1. It sources an optional **machine conf** at
   ...
   `BUILD_CONFIG_MACHINE`) — layer 3. Template:
...
2. It then sources an optional **untracked repo-local conf** at
   ...
   `BUILD_CONFIG_LOCAL`) — layer 4. Template:
   `workflows/scripts/build/build.config.local.sh.example`.
3. It then applies its own `:=` defaults, the **tracked repo conf** — layer
   5, since this file is checked into git and IS the committed default set a
   consuming repo can vendor and edit.
4. A handful of individual machinery scripts (e.g. `pipeline-tick.sh`,
   `pipeline-drive.sh`) additionally keep a matching `:=` fallback for the same
...
after sourcing, and simply assigns the var directly (which — same as layer
...
layers 3 and 4 *before* applying its own layer-5 defaults, and converting
...
(`workflows/scripts/build/pipeline-schedule-gate.sh`,
`workflows/scripts/build/pipeline-overlap.sh`) is the kernel's one control
today: the *path setting* is an ordinary layer-6 kernel default like any other
row in `setting-registry.tsv`, but the *file it points at* is a control — its
...
- **Fail-closed, overriding code defaults at runtime.** A control's reader
  ...
  action, never more (`pipeline-schedule-gate.sh`'s own header documents this
  as the fail-closed/fail-open split with `pipeline-overlap.sh`'s advisory
  ...
  *content* — not just its path — takes precedence over whatever a script
  would otherwise do in the control's absence; the path setting's layer-6
...
`path`-typed row in `setting-registry.tsv` — i.e. a control is only real config
surface if some registered setting's default resolves to it. A file dropped
...
rule (`setting-registry.tsv`'s header) — every operator-overridable seam gets a
row — applied to the *content* layer rather than the *path* layer: the path
setting is registered because it's an ordinary layer-6 default, and the
...
`PIPELINE_SCHEDULE_FILE`'s default resolution is deliberately a two-path
...
the probe entirely and is used verbatim, exactly like every other layer-1/2
override in the ladder above.

## See also

- `workflows/scripts/build/build.config.sh` — the reference implementation
  (layers 3–6) for the build/sweep/pipeline tunables.
...
- `workflows/scripts/build/pipeline-schedule-gate.sh` and
  `workflows/scripts/build/pipeline-overlap.sh` — the kernel's one operator
  control (`PIPELINE_SCHEDULE_FILE`) and its Controls/-then-Context/ default
  resolution.
