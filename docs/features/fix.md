...
paths don't fit that shape: `/build` drives a decomposed epic through a
structured plan, and `/sweep` drains a board's entire pool of ungrouped
Ready issues sequentially. Neither is "just make *this one thing* go." Without a
...
issue is public content in a repository that may be open source. That
denylist is **per-checkout residency, not a global service**: the deny-
pattern data lives inside this checkout, so a consuming or client checkout
that doesn't carry it degrades the scan rather than skipping it silently
— `/fix` surfaces a Degradation notice naming what wasn't scanned, but the
scan itself still cannot run there. A consultant driving `/fix` across
several client codebases should treat each checkout's denylist as scoped
to that client alone; one client's denylist offers no protection to
another's outbound content.
...
driven through the same single-item build mechanics `/sweep` uses; an
issue that already has one open pull request is *adopted* —
...
Every run ends at a terminal disposition — merged and closed, a spike (a
verdict-only investigation item) verdict-closed with a comment, parked on
an open question, or a reported no-op — never at "a PR was opened."
`/fix`'s merge gate is always modal, in any repo it runs in — it never
times out to an automatic merge: it surfaces exactly one merge
...
dependency levels — is refused and redirected to `/triage` → `/assess`
(or `/workshop` for invented work) instead.
