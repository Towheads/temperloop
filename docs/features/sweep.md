...
When a run reviews at least one epic through the epic-closing gate, that
same record also carries how many epics it looked at, how many it actually
closed, and how many it left open — a run that never touches the gate
carries none of the three fields at all, so their absence stays a reliable
"this run never reviewed an epic" marker rather than an ambiguous zero.
