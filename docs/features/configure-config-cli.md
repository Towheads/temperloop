...
The six-layer config precedence ladder (`docs/config-precedence.md`) and the
setting registry (`workflows/scripts/config/setting-registry.tsv`) give this repo
a documented, machine-readable set of ~150 tunable settings — but before this
...
way to ask "what value is actually winning for setting X on this machine right
...
wizard over a small, hardcoded starter set of four operator-facing settings
(`PIPELINE_OPERATOR`, `PIPELINE_DRIVE_CONCURRENCY`, `BUILD_MERGE_GATE_WINDOW`,
...
setting's value from one of three sources, in order: an explicit
...
file at precedence layer 3
(`${BUILD_CONFIG_MACHINE:-$XDG_CONFIG_HOME/temperloop/build.config.sh}`),
upserted line-by-line: an existing `: "${NAME:=...}"` line for a setting this
...
extension, via `setting-registry-lib.sh`), the resolved value and the layer
...
this command RE-DERIVES the answer via clean-subshell layer probes,
cheapest first: is the var already exported in this process's real
environment (layer 2, "env")? Else, does sourcing the machine-conf file set
it (layer 3)? Else the repo-local file (`BUILD_CONFIG_LOCAL`, layer 4)? Else
`build.config.sh` itself (layer 5/6's one physical file)? Else the
...
whole run (not once per setting) and cache every registry-known variable's
resulting value, so the command stays fast despite the registry's size.
Layer 1 (CLI flag) can never win here — there is no live invocation to
...
list` reads `workflows/scripts/config/setting-registry-lib.sh` (the
`setting_registry_rows`/`setting_registry_validate` union API) and
`workflows/scripts/build/build.config.sh` directly; `configure` reads the
same registry lib for its curated settings' default/type/doc metadata and
writes to the exact discovery path `build.config.sh` itself resolves at
layer 3 (via the already-registered `BUILD_CONFIG_MACHINE` setting), so a
`configure` write takes effect the next time any machinery script sources
`build.config.sh` — no further wiring. Neither subcommand introduces a new
setting-registry row: `CLAUDE_BIN`'s existing row (owned by
`pipeline-drive.sh`) already covers this item's identical-literal
...
setting list / validation charsets are plain hardcoded script constants, not
