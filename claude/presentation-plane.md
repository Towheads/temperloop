...
| `/interview` parameter names `--into <note>`, `--first-question <block>`, `--check-questions <block>` | **Frozen** | `claude/commands/interview.md` § Parameters | A caller (`/workshop` Phase 1 today) executes the interview spec inline in the same session with exactly this parameter block; the "call" is a spec read, not a CLI parse, so a renamed or reworded flag desyncs caller prose from callee prose with no runtime error to catch it — the caller passes a parameter the interview never reads, or omits one it requires. |
