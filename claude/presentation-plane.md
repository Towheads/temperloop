...
| `/interview` parameter names `--into <note>`, `--first-question <block>`, `--check-questions <block>` | **Frozen** | `claude/commands/interview.md` § Parameters | A caller executes the interview spec inline in the same session with exactly this parameter block — the designed caller is `/workshop` Phase 1, **pending** the workshop rewrite (temperloop#1958, `workshop-two-phase-rewrite`; on `main` today no command calls `/interview`, it is hand-run only), and the names are frozen *for* that pending caller so it can build against a stable callee; the "call" is a spec read, not a CLI parse, so a renamed or reworded flag desyncs caller prose from callee prose with no runtime error to catch it — the caller passes a parameter the interview never reads, or omits one it requires. |
