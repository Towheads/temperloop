...
| Telemetry / raw-lake record shapes (`command-run`, `issue-touches`, `claims`, `pipeline`, `knowledge-search-fallback` streams; the `schema_version` convention) | **Frozen** | `meta/data/raw/README.md` (canonical sink spec) | Every emit site (`emit-command-run.sh`, `emit-issue-touch.sh`, `emit-gh-perf.sh`, …) and every reader points here rather than restating the shape; a stream's readers key off exact field names, and a breaking change requires the documented `schema_version` bump. |
