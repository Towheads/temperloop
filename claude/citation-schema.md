...
A marker is an HTML comment appended to an existing line (it renders invisibly in Markdown and adds **zero lines** — the prose-budget caps are untouched by construction; same mechanism as `check-setting-prose.sh`'s `<!-- setting-prose:allow -->`):
...
- **Never on a machine-parsed or frozen line** (`claude/presentation-plane.md`'s index — e.g. the capture-backstop registry table rows in `claude/commands/tidy.md`, frozen template lines): put the marker on the adjacent owning prose instead.
