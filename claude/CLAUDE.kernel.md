...
(plus a rendered knowledge-store-routing section, and any `{{KNOB_NAME}}` placeholder
tokens this kernel doc's own prose contains — § Prose-resident knob convention below)
...
Autonomous merge is **scoped and revocable**, and operator silence is never consent for a decision that has no safe default. Three standing rules — the pipeline's mechanics already implement all three (`claude/commands/build.md` Step 4's regime-selected merge gate; the `blocking-now` / `batch-at-gate` / `batch-at-ritual` partition in `[[Context/foundation - AskUserQuestion severity taxonomy]]`), and these state the *principle* at the standing-rules altitude so it is discoverable, not only the machinery:

- **Timeout ≠ consent for a no-safe-default decision.** Only a decision that *carries* a safe default may auto-proceed when a timed window elapses (the walk-away case, where absence of objection = consent). A `blocking-now` decision — a structurally-risky merge set, a design-fork, a claim conflict — has **no** default by construction, so a timed-out or absent operator is **not** approval: the item parks and reports, it never proceeds on silence. The general principle behind this: a question is a request for a *decision*, not a license to *act* — neither the question's existence nor its timeout authorizes proceeding, and that extends to any external-system write (posting, publishing, pushing), not merges alone.
...
- **Only a clean, disjoint set is timed; a risky set is always modal.** The merge-autonomy boundary is risk-tiered: a clean, disjoint, independent set auto-merges after the `BUILD_MERGE_GATE_WINDOW` window unless the operator objects; a structurally-risky set hard-blocks on an explicit modal approval and is *never* timed-auto-merged. Whether the autonomous merge tier runs at all is itself a knob (`FUNNEL_DRIVE_MERGE` in `workflows/scripts/build/build.config.sh`), named here and never valued (§ Prose-resident knob convention).
...
Every local checkout, worktree, and launchd agent has a **role**, and each role owns its own definition of "clean." Drift accumulates silently otherwise — `build`'s own Step 0.5 recovers only the current run's stranded claims, and a leaked worktree or a cron checkout that quietly fell behind `main` otherwise goes unseen until someone trips over it. The clean-baseline-per-role contract:
...
**The detection substrate.** `workflows/scripts/build/env-reconcile.sh` is the shared, **read-only and fail-open** reconciler: it enumerates every checkout/worktree/launchd agent by role and classifies drift against that role's own baseline, but it never mutates anything itself — no `git fetch`, no `launchctl load/unload`, no file writes. `--format report` prints the full human-readable classification; `--format entry` prints a ready-to-append `### … Status: open` vault block **iff** drift was found, and prints nothing when the environment is clean. `workflows/scripts/env-hygiene-report.sh` is a thin passthrough wrapper over `env-reconcile.sh --format entry` — either invocation is equivalent for the `entry` form.

**The `/tidy` audit surface.** `/tidy` runs a periodic, drain-internal **detect-and-propose** § Environment hygiene step (`claude/commands/tidy.md`) — a probe, not a live/drain pair, since environment drift is accumulated host *state* rather than an author action a live rule captures. It runs both probe formats, then disposes each drift class per the aggressive-in-lane/report-cross-lane split above: a leaked worktree or a confirmed-clean cron checkout's `BEHIND_MAIN`/merged-local-branches get auto-healed on the spot; a foreign operator checkout's drift, or an unloaded/stale launchd agent, is appended verbatim to `Context/pipeline - environment hygiene report.md` (a knowledge-store pipeline surface, parallel to the pending-decisions and sensitivity-flags surfaces) for `/check-in` to review and dispose — `/tidy` never mutates a foreign checkout's `HEAD`. A drift class with no safe auto-heal that keeps recurring, or an auto-heal that itself fails, is filed as a board defect rather than left to re-surface silently every run.
...
Default to running **`/workshop`** before materializing invented, epic-sized work into a hand-authored epic. "Invented" means born in a conversation — an idea, a new capability, a "we should build X" — as opposed to *discovered* work (a Backlog defect, a sweep finding), which still enters through `/triage` as before; `/workshop` is the funnel's **second front door**, for invented rather than discovered work (`[[Decisions/temperloop - design command as front door for invented work]]`). "Epic-sized" is the same threshold § Task workflow's "Decompose epic-sized work up front" rule already names (`{{EPIC_MIN_SUBUNITS}}`+ parallelizable sub-units, or more than one dependency level) — below it, a single board item is right and `/workshop`'s coverage walk is unneeded overhead.
...
- **Set a worker or fan-out agent's model tier *explicitly* to the cheapest tier that fits the work's judgment level**, rather than defaulting every agent to whatever tier the driver runs. Mechanical, high-volume, or low-judgment stages route to the cheaper tier; genuinely high-judgment stages (a merge decision, an architectural fork) route to the stronger tier. **Which** model each tier maps to is a knob, never a literal in this prose (§ Prose-resident knob convention): the autonomous funnel driver's two model knobs — `FUNNEL_DRIVE_MODEL` (mechanical drives) and `FUNNEL_DRIVE_MERGE_MODEL` (high-judgment code/merge drives) in `workflows/scripts/build/build.config.sh` — are the worked example, and a new fan-out site names its own model knob the same way. The failure this prevents is a fan-out that silently launches many agents on the top tier for work that never needed it — the default-to-strongest cost leak an explicit, config-named tier closes.
