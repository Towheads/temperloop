...
| Board id (`--board N`, the only number spoken in prose) | Name | Repo | Org-project URL |
|---|---|---|---|
...
⚠️ The board ids and the org-project URL numbers are **swapped for 3 and 4** (a migration-order accident the adapter's `board_project_number()` absorbs). Always speak **board ids**; URL numbers appear only inside full URLs, never as standalone identifiers. Per-repo *issue* numbers are a third, unrelated space — qualify them per § Communication conventions. Board 7 registered via `boards.conf` rather than the built-in case map (foundation #808 — see `ISSUES-ONLY-BACKEND.md` § "The temperloop tracker"); an issues-only board needs no Projects board to have a board id, it only needed the `repo` axis wired up. Still refer to it by the `K<N>` shorthand (§ Communication conventions) in prose about *issues*; `--board 7` is the adapter handle, used by scripts (e.g. `capture.sh --repo kernel`).
...
**Board hygiene is part of the gate.** The board is only a trustworthy parallelism substrate if it reflects reality — and **which mechanism keeps it that way is backend-dependent; do not assume the cascade**. On a **Projects-v2** board the close→Done cascade (GH #340) is a real GitHub-side automation: the merge's `Closes #N` (or an `gh issue close`) closes the issue and the cascade moves the card, so there a manual `board_set_status … Done` is a redundant backstop, not the primary mechanism — rely on the cascade and let `reconcile.sh` catch the rare card it misses. On the **issues-only** backend (the default) there is **no such automation and nowhere to hook one**: the close makes the item *read* Done, but Done is defined as closed **with no** `fnd:status:*` label, and only the adapter's Done write strips that label (and the `fnd:host/session:*` claim stamp). So there **`board_set_status … Done` is the PRIMARY mechanism, not a backstop** — owed by whoever closes the item, including after a merge's own `Closes #N` — and `reconcile.sh --status` / `--labels --apply` is the sweep for closes that bypassed the adapter (K#902: 9 of 9 closures carried by the manual move). When you create an epic's sub-issues, add them to the board. A board that shows closed work as Ready/In-Progress can't be used to pick parallel work. If you find drift, fix it before pulling new work. (This composes with **Fetch ground truth before building** — probe the board's real state before relying on it.) <!-- cite: K.41 incident:F#130 -->
