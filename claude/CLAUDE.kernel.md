...
**The detection substrate.** `workflows/scripts/build/env-reconcile.sh` — read-only, fail-open — is the shared drift reconciler; its roles and its two probe formats (`--format report` / `--format entry`) are owned by its own usage header (`workflows/scripts/env-hygiene-report.sh` is its thin `--format entry` wrapper).
