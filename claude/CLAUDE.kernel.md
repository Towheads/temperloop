...
**Enqueue-method caveat (step 5, manual path).** Some queue configs reject `gh pr merge --merge`; the correct enqueue mechanics (bare enqueue + programmatic confirm) are owned by `workflows/scripts/board/pr-enqueue.sh` (its own header) — prefer `pr-enqueue` and sidestep the manual path entirely.
...
**The detection substrate.** `workflows/scripts/build/env-reconcile.sh` — read-only, fail-open — is the shared drift reconciler; its roles, formats, and flags are owned by its own usage header (`workflows/scripts/env-hygiene-report.sh` is its thin `--format entry` wrapper).
