...
- **Timeout ≠ consent for a no-safe-default decision.** Only a decision that *carries* a safe default may auto-proceed when a timed window elapses (the walk-away case, where absence of objection = consent). A `ask-now` decision — a structurally-risky merge set, a design-fork, a claim conflict — has **no** default by construction, so a timed-out or absent operator is **not** approval: the item parks and reports, it never proceeds on silence. The general principle behind this: a question is a request for a *decision*, not a license to *act* — neither the question's existence nor its timeout authorizes proceeding, and that extends to any external-system write (posting, publishing, pushing), not merges alone. <!-- cite: K.18 class:unbounded-autonomous-writes -->
