...
1. **List the priorities notes.** List `Projects/*/Priorities.md` (`mcp__obsidian-builtin__vault_list "Projects"`, filtering to each project's `Priorities.md`); if the `Projects/` folder doesn't exist yet (pre-overlay-move checkout), fall back to `mcp__obsidian-builtin__vault_list "Priorities"` — one `Priorities/<project>.md` per active project (foundation, stageFind, ssmobile, subsetwiki, …). Skip `_template.md`.
