...
Read the **unattended pending-decisions surface** — `Pipeline/pending decisions.md`, falling back to the legacy `Context/pipeline - pending decisions.md` (path fallback convention above) — via `mcp__obsidian-builtin__vault_read`. This is the cross-run inbox where `ask-at-checkin` questions land when a batch-pipeline command (`build` Step 1.5/1.7/4b/4d, `assess` Step 1's provenance default, `assess` Step 6, `tidy`'s stale-claim sweep, `tidy`'s provenance-less-epics sweep, `tidy` Step 0's stalled-archive-PR heal, `sweep` Step 2) ran **unattended / mini / cron** with no live operator and took its safe default to keep moving (see `claude/CLAUDE.md` § Unattended pending-decisions surface).
