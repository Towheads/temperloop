...
`/workshop` is the funnel's second front door (invented work, not discovered
...
- **Verify premises against `origin/main`, never a stale parent checkout.** The orchestrator's own greps/reads *and* the Step 3 sanity-check subagents must judge every decomposition premise (does a file/symbol exist, what does the code actually say, is a member already fixed) against **freshly-fetched `origin/main`** — not the parent working tree, which can sit many commits behind while `/build` workers branch off fresh `origin/main`. Concretely: `git fetch origin main` first, then read/grep a fetched ref (`git show origin/main:<path>`, `git grep <pat> origin/main`) rather than the on-disk tree. A premise checked against a stale checkout produces wrong sub-issue contracts and false re-triage signals — the auditor once declared a test file nonexistent that a live worker was editing, because the parent tree was 27 commits behind (`[[Mistakes/foundation - assess premises verified against a stale parent checkout]]`). This is the read-side complement to the branch-side `git-stale-branch-guard.sh` hook: that guard catches *branching off* a stale local `main`; this rule catches *reasoning about* a stale tree, which no PreToolUse hook can distinguish from legitimate work.
...
- **A decision-queue-parked issue is an external gate, never a plan item.** When decomposition surfaces an issue that is already parked in the **decision queue** — carrying the `decision` label and assigned to the operator (`~/.claude/decision-queue-contract.md`) — do **NOT** mint a plan item (or a spike) for it. It is not work *this* plan drives; it is an external **dependency** whose answer resumes on its own track (the answer-drain machinery — `/build` Step 0a — picks it up when the operator replies and unassigns). Record it as an **external gate**: name it in `## Sequencing notes` / `## Re-triage signals`, and put a `gate_check:` predicate on the item that *consumes* its outcome (per the `gate_check:` principle above — on the **consumable, not the tracker**). Minting a plan item for it would double-drive an issue the decision queue already owns. See [[Patterns/foundation - Decision-queue blockers are external gates not plan items]].
...
3. Resolve the board/repo from `--board`/`--project`. **If neither was given, infer the board from the local repo** (matching `/triage` Step 0.3, foundation#547): set `BOARD_LIB` = the first of `scripts/lib/board.sh` or `workflows/scripts/board/lib/board.sh` that exists, `source "$BOARD_LIB"`, then `repo=$(gh repo view --json nameWithOwner -q .nameWithOwner); BOARD=""; for b in $(board_registered_boards); do [ "$(board_repo "$b")" = "$repo" ] && BOARD="$b"; done` — the candidate list is the adapter's own `board_registered_boards` (the single source of truth; board **7** is the temperloop kernel tracker, an issues-only backend — see `workflows/scripts/board/ISSUES-ONLY-BACKEND.md` — and resolves through the same reverse-lookup as any other board; a hardcoded `3 4 5 6` here is the temperloop#352 drift that dropped it). If a match is found, print `inferred board $BOARD (repo $repo)` before any board read and continue; if **no** candidate matches (an unmapped repo), STOP with `/assess: cannot infer board — pass --board <N> (3=stageFind, 4=foundation, 5=ssmobile, 6=subsetwiki, 7=temperloop kernel) or --project <name>, or run from a board-mapped repo`. Call the resolved board number `$BOARD` (the `## Re-triage signals` persistent route in Step 4 reads it). Confirm the epic exists: `gh issue view <N> -R <repo> --json number,title,state` — stop if it's missing or closed.
4. Confirm the plan-note schema `~/.claude/plan-schema.md` is reachable (deployed to `~/.claude/plan-schema.md` from `claude/plan-schema.md` by `make install-claude` in the foundation fleet). If missing, surface "schema reference missing — run `make install-claude` from the foundation checkout, or copy `claude/plan-schema.md` to `~/.claude/plan-schema.md` directly on a standalone kernel checkout (this repo's Makefile deliberately carries no `install-claude` target)" and stop.
...
- **Contract present → epic-decomposition mode** (the pre-designed, intentionally-undecomposed epic). **First, the provenance check (temperloop#218):** scan the epic body for a `design-brief: [[Designs/<note>]]` marker line (`claude/design-schema.md` § Materialization contract — the marker `/workshop`'s Step 5a materialize writes). **Marker present** → this Contract came from a ratified `/workshop` walk; proceed silently, no ask. **Marker absent** → this Contract was hand-authored, not materialized from `/workshop` — ask the operator via `AskUserQuestion`: *"Epic #<N> carries a `## Contract` with no `design-brief:` marker — it wasn't materialized from a ratified `/workshop` walk. Proceed with this hand-authored Contract as-is, or park this epic and run `/workshop` first to walk it against the full coverage template (`claude/design-schema.md`)?"* — **Proceed** (default) or **Park and run `/workshop`**. **This ask is fail-open, never a hard stop:** on an unattended run, or any run with no operator to answer, take the default (**Proceed**) — never block the run waiting on this question. **This is a `batch-at-ritual` deferral** (`claude/CLAUDE.md` § Unattended pending-decisions surface) — a safe default with no live operator — so don't *silently* default: record the auto-taken default by appending one `### open` entry to the pending-decisions surface (`Pipeline/pending decisions.md` vs the legacy `Context/pipeline - pending decisions.md` — target pinned by the append-target resolution rule of the path fallback convention, `claude/commands/check-in.md`; in the knowledge store) via `mcp__obsidian-builtin__vault_append`, so the next `check-in` reviews it (its Pending decisions review section is the reader):

  ```markdown
  ### <YYYY-MM-DD HH:MM> · assess Step 1 provenance default · epic #<N>
  - **Decision:** epic #<N> ("<title>") carries a `## Contract` with no `design-brief:` marker — proceed with the hand-authored Contract, or park and run `/workshop` first?
  - **Default taken:** Proceed (decomposed the hand-authored Contract as-is)
  - **Disposition:** auto-taken (unattended; no live operator)
  - **Status:** open
  ```

  Also note the auto-taken default in the Step 5/6 summary (the in-run trace; the surface entry is the durable one). **On Park:** post a one-line durable comment on the epic issue so the state survives the session — `gh issue comment <N> -R <repo> --body "parked pending /workshop — /assess Step 1 provenance check: this epic's ## Contract carries no design-brief: marker; run /workshop against it, then re-run /assess --epic <N>"` — leave the epic's board Status untouched, note "parked pending `/workshop` — epic #N" in the Step 5/6 summary, and stop this command's run for that epic (the operator runs `/workshop` against it, then re-runs `/assess --epic N`; if the operator never acts, `/tidy`'s § Provenance-less epics sweep is the fallback net that re-surfaces the epic at the next `check-in`). **On Proceed (or the fail-open default):** continue below — do **not** stop. Parse the epic body's `## Contract` into the source for Step 2:
...
Pass the **effective (merged) set** to **every** review subagent below as **additional evaluation criteria** ("also judge each item against these principles; flag any that violate one — cite the principle's name and origin, `kernel` or `project`, in any finding" — this is what makes a citation traceable to a named principle and makes an excluded principle's citation disappear, since an excluded principle is never in the set handed to the reviewer). The principle notes are the canonical source; each kernel principle and each project principle note carries (or, for the kernel file, states inline) the bar a reviewer judges against; a project's own principle note carries an `## Enforced by` footer naming the CI gate + agent that operationalizes it.

**Provenance is never silent — carry it forward for Step 5.** Whatever the resolution above lands on, name it explicitly so "kernel set applied" and "project declined it" never look the same in the output: the kernel source (`claude/engineering-principles.md`, 7 principles) is always cited; the project's resolved identity (`Projects/<project>/Priorities.md`, the legacy path, or "absent") is always named; and the merge mode in effect (extend / `mode: replace` / named exclusion(s) / `none`) is always stated. This is the `principles:` line Step 5 renders — compute it once here, don't re-derive it there.
...
- **Always add the `principles:` notice line** — the provenance Step 3 resolved, never omitted (the kernel set always applies, so there is no "principles absent" case to elide): `kernel (7) only — no project § Principles declared`, `kernel (7) ∪ project (<resolved path>: <M>, phrasing wins on <D> dupe(s)[, override(s): <name>])`, `kernel (7) only — project § Principles present but empty/malformed, treated as absent`, `project (<resolved path>: <M>) — mode: replace, kernel set discarded`, or `none — project opted out (<resolved path> § Principles: none)`.
...
- **Contract present with no `design-brief:` marker (Step 1 provenance check, temperloop#218).** Ask (proceed / park + run `/workshop`) — never a silent bypass, never a hard block. Fail-open: an unattended run or an absent operator takes the **Proceed** default, records it as a `### open` entry on the pending-decisions surface (the `batch-at-ritual` convention — never a silent default), and notes it in the summary rather than stalling. A `park` answer posts the durable "parked pending /workshop" comment on the epic, then stops this command's run for that epic, not the whole invocation (a multi-epic batch, if any, continues); `/tidy`'s § Provenance-less epics sweep re-surfaces a parked epic the operator never returns to.
