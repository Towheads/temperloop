...
- **Contract present with no `design-brief:` marker (Step 1 provenance check, temperloop#218).** Ask (proceed / park + run `/design`) — never a silent bypass, never a hard block. Fail-open: an unattended run or an absent operator takes the **Proceed** default and notes the gap in the summary rather than stalling. A `park` answer stops this command's run for that epic, not the whole invocation (a multi-epic batch, if any, continues).
