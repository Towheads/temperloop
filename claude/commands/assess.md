...
- `--board <N>` / `--project <name>` (optional) — which board/repo the epic lives on. stageFind = `3` (repo `<org>/stageFind`), foundation = `4` (repo `<org>/foundation`). **Default: stageFind (`3`)**, consistent with `/triage`. The project slug for the plan filename/tag is derived from the board (3 → `stagefind`, 4 → `foundation`) unless `--project` overrides.
...
3. Resolve the board/repo from `--board`/`--project` (default stageFind `3` / `<org>/stageFind`); call the resolved board number `$BOARD` (the `## Re-triage signals` persistent route in Step 4 reads it). Confirm the epic exists: `gh issue view <N> -R <repo> --json number,title,state` — stop if it's missing or closed.
