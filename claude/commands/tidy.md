...
**For every accepted extraction with `method: "drain-model-skim"`** (the model caught it, the lexicon didn't), append one entry to the candidate-tells file at `Pipeline/candidate tells.md`, falling back to the legacy `Context/pipeline - candidate tells.md`, in the vault via `mcp__obsidian-builtin__vault_append`.
