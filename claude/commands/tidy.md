...
2. List `Sessions/_inbox/` via `mcp__obsidian__list_vault_files`. **Only `*.md` files are stubs** — ignore any non-`.md` entries. (The MCP listers hide dotfiles, so the `.drain.lock.*` locks never appear here anyway — the election enumerates them **off disk** in 4d, not through an MCP lister; F#1199.) If there are no `*.md` stubs, say so in one line and exit (don't acquire the lock — there's nothing to race over).
...
4. **Acquire the cross-machine drain lock.** Multiple machines share `Sessions/_inbox/` via Obsidian Sync, so two `/tidy` runs (e.g. evening rituals on two machines within a minute of each other) can process the same backlog at once and double-create Things tasks (the Step 4 dedup is search-then-add, not atomic across concurrent runs). Sync is eventually-consistent (~seconds to a minute), so a plain lockfile is racy — the protocol is **acquire → wait for Sync → elect**, earliest timestamp wins:
   ...
   d. Enumerate and read the `.drain.lock.*` files **off disk via Bash — not an MCP lister**: both `mcp__obsidian__list_vault_files` and `mcp__obsidian-builtin__vault_list` hide dotfiles, so a re-list through either returns **zero** locks and every host would elect itself and double-drain — the exact race this lock exists to prevent (F#1199). Read them directly under the knowledge store root instead — `for f in "$KNOWLEDGE_STORE_ROOT"/Sessions/_inbox/.drain.lock.*; do [ -e "$f" ] && echo "$f: $(cat "$f")"; done` — a raw on-disk read of ephemeral coordination metadata (same disk-not-MCP precedent as Step 1's `scan_stub.py` and Step 7's archiver; the MCP-only rule governs knowledge notes, not the lock file). **Discard any lock whose `<EPOCH>` is older than `TIDY_LOCK_STALE_AFTER`** (a crashed prior run — never let it block forever) and delete its file via `mcp__obsidian__delete_vault_file` by its path — the Bash read above is only because the MCP listers can't *see* the dotfiles; deletes still route through MCP, matching 4e and Step 7. Among the rest, the winner is the **lowest `<EPOCH>`**; tie-break on the lexicographically smallest `<HOST>`.
...
2. Run the archiver **once, passing every collected stub path as a separate argument**. It copies each into `meta/sessions/archive/<basename>` via `cp` (printing `archived: <basename>` per stub), then runs the retention sweep (gzip cold logs), regenerates `INDEX.md` **once**, and makes a **single** landing attempt for the whole batch:
   ...
   **(a) One `archive-stub-durable:` / `archive-stub-pending:` line PER STUB** — whether *that* stub's transcript is individually on `origin` right now (proved by exact bytes: the stub's `git hash-object` against the archived blob on `origin/<default>`). These govern **deletion**:
   - `archive-stub-durable: <basename>` — this transcript is on `origin`; the `_inbox` stub is safe to delete.
   - `archive-stub-pending: <basename>` — not proven durable; keep it.
   ...
   **(b) The FINAL stdout line: one durability verdict for the whole BATCH.** This governs only what you **report**, never what you delete (the per-stub `archived:` lines above it are progress, not a verdict):
   ...
   - `archive-pr-queued: <pr>` — the batch was landed on **one** PR that is **enqueued but not yet merged** (protected `main`). The queue merges it asynchronously, minutes later.
...
3. **Delete per stub, report per batch.** Delete each stub the archiver marked `archive-stub-durable:` (via `mcp__obsidian__delete_vault_file`, one call per stub) — **whatever the batch verdict says**. Keep every `archive-stub-pending:` stub; deleting one before its transcript lands would lose it. If the archiver emitted no per-stub lines at all (an older build, or a path where `origin` can't be consulted), fall back to the batch line: delete all only on `archive-committed`, else keep all.

   Then report the batch verdict in the summary:
   - `archive-pr-queued: <pr>` — list it in the Step 6 summary as `archive PR #<pr> queued — N stubs retained until merge`. This is **expected, not stuck**: the queue lands the PR async, and a later run releases those stubs once it can prove them on `origin`.
   ...
   **Why per-stub, not per-batch** (#372; downstream evidence in the foundation overlay's #1161): the archiver folds a whole run into ONE commit/PR (#487), so a batch containing *any* genuinely-new stub has real changes to land and returns `archive-pr-queued` — which under the old batch-governs-deletion rule retained **every** stub in it, including ones merged to `origin` days earlier. Since every batch on an active machine contains something new, "retain" became the permanent verdict and `_inbox` never drained: 109 of 123 stubs stranded over 11 days, behind two archive PRs that had **both already merged**. This spec previously claimed the flow "self-heals across runs" once the PR merges — it does not, and that claim has been removed rather than reworded: self-healing needs a batch containing *nothing* new, which never arrives.
