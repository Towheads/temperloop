...
Also carry forward, per **group whose members are ALL classified `Operational`** (see the member-class consistency check at Step 3 below — a mixed-class group carries no `logical_order` at all until it is resolved), an optional **`logical_order`** list of `(<blocked-member>, <blocker-member>)` pairs — genuine **meaning-level** precedence between members ("the schema item precedes its consumers"), judged the same way group-by-meaning itself is judged (Step 2.3/2.4). This is deliberately narrow and advisory, per the amended invariant (§ Operating principles, docs/adr/0031): <!-- cite: TR.23 guard:docs/adr/0031-durable-logical-order-lives-on-the-board-as-blocked-by.md -->
     ...
     case "$verdict" in
       SAFE\ *)
         if board_blocked_by_add "$BOARD" <blocked> <blocker>; then
           gh issue comment <blocked> -R "$repo" --body "blocked_by #<blocker> — logical order (triage): <one-line rationale>"
