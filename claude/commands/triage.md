...
Items 1, 4 and 5 are skipped: item 1 (Obsidian tools) serves doc intake and decision-route notes, item 4 (`board_resolve`) is the whole-board read the sweep needs, item 5 validates doc paths — Step 6 uses none of them. Skipping item 4 in particular is the flag's whole point: it is the expensive whole-board read, and Step 6 needs only `board_repo` (a static map lookup) plus per-item `board_parent_issue` (REST, no prior resolve).
