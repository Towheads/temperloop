...
1. **Reclaim the worktree (in-lane, idempotent)** — for a **merged** or **resolved (verdict)** disposition that built in a worktree (the 4a fresh drive; not the 4b adopt path, which owns no local worktree): `workflows/scripts/build/worktree.sh remove "$repoRoot" <slug>`. Idempotent by construction — `cmd_remove` returns `REMOVED` when a worktree/`build/<slug>` branch existed and `NOT_FOUND` otherwise, so a re-run (or the deploy-mini session-start sweep, F#653, the crash-path backstop) is a safe no-op.
   ...
   **Delete the preservation ref — the run that pushed it owns it.** If this run pushed `build/<slug>` to origin (4a's inline-answer escalation branch, item 3), delete it in the same step: `git -C "$repoRoot" push origin --delete "build/<slug>"` (harmless if it is already gone). That ref exists only to carry committed work across a `worktree.sh create`, and a **merged** / **resolved (verdict)** disposition means the work has landed — leaving it behind is a stray remote branch nothing owns. **On a parked or held disposition it is deliberately KEPT**, and that is not a leak: the park path removed the worktree and with it the local `build/<slug>`, so the pushed ref is then the *only* surviving copy of the escalated work. The park comment (4a park step 2) names it, and the next `/fix` run on this issue deletes it here, at *its* terminal disposition.
