...
Workflow({ scriptPath: workflowPath, args: {
  ...
  items: [ {
    ...
    notes: <any operator clarification already captured this run, else "". On a re-drive after an
                     //   inline-answered escalation this ALSO carries the restore instruction for the
                     //   preservation ref the escalation branch pushed — see the `escalations:` bullet below>
  } ],
  verdicts: {}, onlySlugs: []
} })
```
...
- **`escalations: [{ slug, kind, payload }]`** (the worker hit a question / blocker — `blocked` / `design-fork` / `failed` / a machinery escalation) → **park + stop** (a terminal **parked** disposition for this single-item run; `/fix` drives *one* item, so a worker escalation ends the run — resume = re-run `/fix` on the same target after answering, **never an in-place continuation**, since `/fix` always invokes `build-level.mjs` with `verdicts: {}, onlySlugs: []`, i.e. never `isContinuation`). Surface the escalation via `decision_sink_ask(<the payload's question / design_fork.decision+options / failure_reason>, <per-kind options>, ask-now)`, then take one of the two branches below.

  **What the next `worktree.sh create` destroys — this governs BOTH branches.** `build-level.mjs` leaves an escalated worktree **intact**, and any later drive of the same slug calls `worktree.sh create`, whose first act is `clear_path`: it force-removes `<repoRoot>.wt/<slug>` **and `git branch -D`s the local branch `build/<slug>`**, then re-adds that branch off a freshly-fetched `origin/<default>`. So it discards the escalated worker's uncommitted edits **and every commit it made** — committing is *not* protection, and no other ref in that same repo (a tag, a stash, a second local branch) survives either, because `clear_path` operates on that repo. Only a ref **outside** the local repo does. <!-- cite: FX.8 class:escalated-work-destruction -->

  - **Operator answers inline** → **preserve the escalated attempt's work FIRST, then re-drive.** The operator being present is exactly when real work exists on disk, so this branch — not the park branch — is the common one:
    1. **Probe what is there** — `pr.sh recover-probe "<repoRoot>.wt/<slug>" "build/<slug>"`. Read `.commits_ahead` (commits on the escalated branch) and `.dirty` / `.dirty_files`. The field is `commits_ahead`, **not** `ahead` — `ahead` is only the shell variable inside `cmd_recover_probe`; the emitted JSON key is `commits_ahead` (`workflows/scripts/build/pr.sh:442`, documented in that script's usage header at `pr.sh:70`), and querying `.ahead` returns `null`, which fails every comparison below and falls through to an unnecessary commit/push instead of the intended skip. `commits_ahead: 0` with `dirty: false` → there is nothing to preserve; skip to item 4.
    2. **Sweep uncommitted edits into a commit** (only when `.dirty`) — `git -C "<repoRoot>.wt/<slug>" add -A && git -C "<repoRoot>.wt/<slug>" commit -m "wip: escalated attempt (<slug>)"`. `clear_path` deletes the working tree as well as the branch, so an edit left uncommitted cannot be pushed and cannot survive.

       **A nonzero `commit` STOPS the run — it does not fall through to item 3.** This step has the same standing as item 3's `PUSH_REJECTED`, and for the same reason. If the commit fails — an index lock, a full disk, a rejecting hook, or an `add -A` that staged nothing — `HEAD` never advances past `origin/<default>`, so item 3 would push a ref *identical to the base* and report a perfectly healthy `PUSHED`. That is a **false positive**: the push "succeeded" while the uncommitted edits it exists to protect were never captured, and the next step destroys the worktree holding them. Same silent-loss class this whole branch closes, one line earlier than the push path. So on a nonzero exit: **report and park — do not push, do not re-drive, and call neither `worktree.sh create` nor `worktree.sh remove`.** The mechanical tell, since a shell exit code can be swallowed: **re-run `recover-probe` and treat `dirty_files > 0` after a commit attempt as proof the commit did not take**, whatever the exit code claimed. This guard is inherited wholesale by the park branch below, whose step 1 runs these same items 1–3.
    3. **Push it to origin as the preservation ref** — `pr.sh push "<repoRoot>.wt/<slug>" "build/<slug>"`. The ref name is the worktree's own `build/<slug>`, deliberately **not** the item's `<type>/<slug>` PR branch, so it can never collide with the re-drive's own push target nor be mistaken for the PR's head; it opens no PR and (`ci.yml` runs `push` only on `main`) triggers no CI. `PUSHED` is the confirmation. A `PUSH_REJECTED` **stops the re-drive** — report it and park rather than calling `worktree.sh create` with the work unpreserved. A second cycle re-pushes the same ref as a plain fast-forward, since the re-drive's branch descends from it — never `--force`.

       **Record the pushed SHA — it is the operand Step 6's deletion gate tests.** `pr.sh push`'s `PUSHED` payload is `{"outcome":"PUSHED","sha":…,"branch":…,"forced":…}`; keep its `.sha` as this run's **`<preserved-sha>`**, in the same in-flight run state that already carries the `pushed_sha` `build-level.mjs` returns at 4a and the `<pr>` Step 5 merges — no state file, no new setting, just one more value the run holds from here to its terminal disposition. If it is ever lost, `git ls-remote origin "refs/heads/build/<slug>"` re-reads the identical commit off the ref that is about to be deleted, so the gate is never left with one operand.
    4. **Re-run the drive from Step 2.** The answer rides into 4a's item `notes:`; when item 3 pushed a ref, `notes:` **also** carries the restore instruction as the worker's FIRST action, before any other work — `git fetch origin build/<slug>` then `git merge --ff-only FETCH_HEAD` — plus one line naming what that commit already contains and what the answer changes about it. `worktree.sh create` branches the new worktree off `origin/<default>` and the preserved commit descends from that same base, so the fast-forward always applies. The second worker then **builds on the reviewed work instead of re-deriving it**.
  - **Otherwise park the target so a later re-run is lossless** — mirror `/sweep`'s escalation park exactly (**never leave the claim held or the worktree as debris** — a still-held claim makes a same-session re-run route back through `fresh` and into the `clear_path` described above; a cross-session re-run instead reads `claimed-elsewhere` and 4d stops, blaming a dead prior run):
    Run these **in the order emitted**, and note that no step below instructs an action belonging earlier in the sequence — the preservation decision is step 1 precisely because step 5's `worktree.sh remove` is irreversible and step 3's park comment must be able to name the ref it pushed:

    1. **Preserve the escalated attempt's work, or decide there is none — FIRST, before anything below.** Step 5's `worktree.sh remove` deletes the `build/<slug>` branch as well as the worktree, discarding the escalated attempt's commits exactly as `clear_path` would. So run the inline-answer branch's items 1–3 above (`recover-probe` → commit-if-dirty → `pr.sh push`) **now**, including both of their stop conditions: a `commits_ahead: 0`, `dirty: false` probe means there is nothing to preserve and this step is a no-op; otherwise the push yields the preservation ref `build/<slug>` and its `<preserved-sha>`. **A failed commit (item 2) or a `PUSH_REJECTED` (item 3) stops the park here** — report it and leave the worktree in place — rather than continuing to step 5 and removing a worktree whose work is unpreserved.
    ...
    3. `gh issue comment <issue> -R "$repo" --body "Parked by fix — <the question from payload>. Where it stands: <one line>. Re-run /fix <issue> once answered."` — post the question **FIRST**, before the handled markers below (a labeled+assigned issue with no question comment is silent loss; if this comment fails, do **not** proceed to step 4 — leave it un-flagged to re-park on the next run). When step 1 pushed a ref, **name it and its SHA in this comment** (`Escalated work preserved at origin/build/<slug> (<preserved-sha>).`) — after step 5 that ref is the only surviving copy, and this comment is the operator's only lead to it.
    ...
    5. `"$RELEASE" <issue> --board "$BOARD"` (clear the claim marker), then `workflows/scripts/build/worktree.sh remove "$repoRoot" <slug>` — the workflow leaves an escalated worktree **intact**; `/fix` discards it (resume = re-run, not in-place continuation), so remove it now. This is what prevents the next run's `worktree.sh create` from force-clearing an orphaned worktree — the lossless-re-run guarantee. **`remove` deletes the `build/<slug>` branch too** — which is safe here only because step 1 already ran.
    6. Record the target as **parked** (with the reason + the `needs-clarification` marker carrying it forward) and **stop** at Step 7.
...
1. **Reclaim the worktree (in-lane, idempotent)** — for a **merged** or **resolved (verdict)** disposition that built in a worktree (the 4a fresh drive; not the 4b adopt path, which owns no local worktree): `workflows/scripts/build/worktree.sh remove "$repoRoot" <slug>`. Idempotent by construction — `cmd_remove` returns `REMOVED` when a worktree/`build/<slug>` branch existed and `NOT_FOUND` otherwise, so a re-run (or the deploy-mini session-start sweep, F#653, the crash-path backstop) is a safe no-op.
   ...
   **Delete the preservation ref — but ONLY after proving the work it holds actually landed.** At a **merged** or **resolved (verdict)** disposition, probe for the ref **unconditionally** — *whether or not this run is the one that pushed it*:

   ```bash
   git -C "$repoRoot" ls-remote --exit-code origin "refs/heads/build/<slug>"
   ```

   Exit non-zero → no preservation ref exists for this slug; skip the rest of this sub-step. Exit 0 → the printed SHA **is** `<preserved-sha>` for the purposes of the gate below, and the ref is disposed exactly as follows.

   The probe is unconditional because the common case is **cross-session**: the escalated run pushed the ref and parked (4a's park branch step 1), and the *later* run that resolves the issue cleanly pushed nothing at all. A condition scoped to "this run pushed it" would never look, so the ref would survive every subsequent clean resolution and the cleanup would exist only in prose. Broadening is safe precisely because the ancestry gate below — not the probe — is what protects the ref: a commit that is not an ancestor of the merged tip is KEPT no matter which run pushed it.

   That ref may be the last copy of reviewed work, so its deletion is **gated on a mechanical ancestry check, never on the disposition alone**. A merged disposition only proves *a* PR merged; it does **not** prove the re-drive's worker actually performed the `git merge --ff-only` restore that 4a's `notes:` instructed. That instruction is free-text prose handed to a model, and silent plan drift is a known failure mode — a worker that skipped it still produces a green, merged PR, so deleting on the merge alone would destroy the reviewed commit *silently and unrecoverably*. Gate the irreversible step instead:

   ```bash
   git -C "$repoRoot" fetch origin "build/<slug>"   # ensure <preserved-sha>'s object is present locally
   git -C "$repoRoot" fetch origin "<default>"      # LAST fetch wins FETCH_HEAD = the merged default tip
   git -C "$repoRoot" merge-base --is-ancestor "<preserved-sha>" FETCH_HEAD
   ```

   (Two separate fetches, in that order, deliberately: `FETCH_HEAD` resolves to the **first** ref of a multi-ref fetch, so fetching both in one command would test ancestry against the preservation ref itself and pass vacuously.)

   `<preserved-sha>` is the SHA the `ls-remote` probe above just printed — which for a run that pushed the ref itself equals the `.sha` it recorded at 4a item 3, and for the cross-session case is simply read off the ref. The test asks the only question that matters: **is the preserved commit an ancestor of the merged default tip?**

   - **Exit 0 (TRUE — it landed)** → delete the ref: `git -C "$repoRoot" push origin --delete "build/<slug>"` (harmless if already gone). It has served its purpose and is otherwise a stray remote branch nothing owns.
   - **Exit non-zero (FALSE — it did NOT land), or the check could not run** (either operand missing, the fetch failed, or the disposition is **resolved (verdict)** with no merge commit and therefore no merged tip to test against) → **KEEP the ref** and **report the discrepancy loudly** in Step 7: name `origin/build/<slug>`, `<preserved-sha>`, the merged `<pr>`, and state plainly that the preserved commit is **not** in the merged result and someone must look. This is never advisory and never silent-on-failure: an unevaluable check is treated exactly as a FALSE, because the cost of keeping a stray ref is a one-line cleanup and the cost of a wrong delete is unrecoverable.

   **Step 6 is the right and sufficient place for this gate.** It is the irreversible step, and gating only the irreversible step is this codebase's own design discipline (§ Design discipline — subtraction over mechanism). The alternative placement — an acceptance assertion inside the re-drive that the restore happened — would run inside the very worker whose compliance is in doubt, so it can be skipped by the same drift it is meant to catch; it is at best a complement, never a substitute, and is deliberately not added here rather than paying for a second mechanism that cannot close the hole on its own.

   **On a parked or held disposition the ref is deliberately KEPT** — and that is not a leak: the park path removed the worktree and with it the local `build/<slug>`, so the pushed ref is then the *only* surviving copy of the escalated work. The park comment (4a park step 3) names it. Because the probe above is unconditional, the next `/fix` run on this issue **does** find that ref when it reaches a merged/resolved disposition — even though that run pushed nothing itself — and disposes it here through this same gate. That is the cleanup path, and it is instructed, not assumed.
