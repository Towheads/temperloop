...
Workflow({ scriptPath: workflowPath, args: {
  ...
  verdicts: {}, onlySlugs: []
                         ...
                         // 4a's resume-in-place arm (an escalation with a commit already ahead
                         // of base) re-invokes this same args block with BOTH keys populated;
...
- **`escalations: [{ slug, kind, payload }]`** → **branch on a checkable FACT about the worktree — does a commit exist ahead of the base — and never on the escalation's kind-name.** A kind-name list is the wrong discriminator because it throws away work already earned — the commit sitting in the worktree — to re-earn it from scratch for what is often a few lines. temperloop#1988 (resume-rule review blocking)'s live case is the evidence: `/fix temperloop#1970` (reviewer serial-HIGH loop) escalated `review-blocking` with the worktree clean, the entire fix already committed as `2973a5ca`, and the blocking finding a doc-surfacing gap. <!-- cite: FX.13 incident:K#1988 -->

  **The discriminator, run once, up front.** Against the deterministic worktree `worktree.sh` created for this slug (path `<repoRoot>.wt/<slug>`, branch `build/<slug>`, based on `origin/<default>`):

  ```
  git -C "<repoRoot>.wt/<slug>" rev-list --count "origin/<default>..HEAD"   # >0 → a committed build exists
  git -C "<repoRoot>.wt/<slug>" status --porcelain                          # non-empty → uncommitted work too
  ```
  ...
  - **A commit exists → RESUME IN PLACE** (the arm below). Two kinds *always* land here, because both are raised after the `done` guard: **`acceptance-incomplete`** (§3d — an otherwise-`done` verdict with one acceptance bullet `passed: false`) and **`review-blocking`** at **both** its raise sites (§3e pre-push, §3g CI-fix). They are illustrations of the rule, not a substitute for it.
  - **No commit exists → park**, and discard only once the worktree is **confirmed** to hold nothing. The kinds that *are* the `done` guard firing — **`blocked` / `design-fork` / `failed`** — have no `done` claim behind them, so committed work here is *variable, not absent*: run the check rather than assume.
  ...
  **What has already been VERIFIED differs by raise site — read that off the payload, never off the kind.** This table is a *reporting* distinction only; it is **not** the keep-vs-discard discriminator (that is the commit check above, which every row below passes identically):
  ...
  **The park path** — taken by an escalation the commit check found nothing behind, and by a resume-in-place escalation only when the operator explicitly parks it. Surface the escalation via `decision_sink_ask(<the payload's question / design_fork.decision+options / failure_reason>, <per-kind options>, ask-now)`; if the operator answers **inline**, record the answer and **re-run the drive from Step 2** (the answer rides into 4a's item `notes:`). Otherwise **park the target so a later re-run is lossless** — mirror `/sweep`'s escalation park exactly (**never leave the claim held or the worktree as debris** — a still-held claim makes a same-session re-run route back through `fresh` and `worktree.sh create` **force-clear** the orphaned worktree, silently discarding the escalated worker's uncommitted edits; a cross-session re-run instead reads `claimed-elsewhere` and 4d stops, blaming a dead prior run): <!-- cite: FX.8 class:escalated-work-destruction -->
  ...
  2. `gh issue comment <issue> -R "$repo" --body "Parked by fix — <the question from payload>. Where it stands: <one line>. Re-run /fix <issue> once answered."` — post the question **FIRST**, before the handled markers below (a labeled+assigned issue with no question comment is silent loss; if this comment fails, do **not** proceed to step 3 — leave it un-flagged to re-park on the next run). When step 4's check preserves the worktree, **name its path in this same comment** — that path is the only pointer a human has to work the check said not to destroy.
  ...
  4. `"$RELEASE" <issue> --board "$BOARD"` (clear the claim marker — the board is back at `Ready`, so the cross-session lock must not outlive it), then, **and only behind the discriminator above, re-run immediately before this line**, `workflows/scripts/build/worktree.sh remove "$repoRoot" <slug>`. The workflow leaves an escalated worktree **intact**, so removing a genuinely empty one here is what keeps the next run's `worktree.sh create` from clearing an orphan — the lossless-re-run guarantee. **Remove ONLY on a confirmed-empty reading**: no commit ahead of `origin/<default>` **and** a clean `status --porcelain`. A commit, dirty tracked files, or a check that would not resolve all mean **keep the worktree** and report its path (step 2) — an unestablished verdict is never grounds for deletion. **Never remove on the escalation's kind-name**, and never at all for an escalation taking the resume-in-place arm above: there the claim and the worktree are exactly what the continuation resumes under, so releasing either destroys the work that branch exists to preserve.
...
- **Parked (open question / not-ready / held)** — `#<issue>` → the reason it parked (the worker's question, `reattach`'s not-ready reason, or a declined merge), and which marker carries it forward (`needs-clarification` for a question; the open PR for a held/not-ready adopt) so a re-run resumes it. An escalation with a **commit already ahead of base** is **not** parked by default — 4a resumes it in place in the same run (`acceptance-incomplete`, and `review-blocking` at either raise site) — so it reaches this bullet only when the operator explicitly chose park. Say which way step 4's check went: a confirmed-empty worktree was removed, anything else was **preserved** and its path named, so no parked target ever hides destroyed work behind the word "parked".
