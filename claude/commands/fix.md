...
The target is drivable. Claim is the **first mutating action** (owned by `build-level.mjs` 3a). **Immediately before invoking**, print the **Workflow launch line** template (`claude/message-schema.md`) — `caller: fix`, this being a 1-item level, `round 1` for this fresh drive (a **post-acceptance** escalation re-invokes the same Workflow at `round 2`+ as a 3d-esc-shaped continuation — see Step 4a's escalation branch below; a pre-acceptance one parks instead, so it never mints a round 2) — so a `/fix` run is distinguishable in the transcript from any concurrent `/build`/`/sweep` drive on the same repo (temperloop#903, temperloop#1941). Invoke the saved Workflow as a **1-item level**, exactly as `/sweep` Phase 2 does (invoke by `scriptPath`; `args` delivered as a JSON string the script parses):
...
Workflow({ scriptPath: workflowPath, args: {
  ...
  verdicts: {}, onlySlugs: []
                         ...
                         // A post-acceptance escalation (4a's continuation arm) re-invokes this
...
- **`escalations: [{ slug, kind, payload }]`** → **branch on WHEN in the item's life the escalation was raised** — the discriminator is the 3e.5 acceptance verdict, and the two halves differ in what is at stake. A **blanket** park-and-discard rule is wrong because it throws away a *finished* build — acceptance passed, the commit already in the worktree — to re-earn it from scratch for what is often a few lines. temperloop#1988's live case is the evidence: `/fix temperloop#1970` escalated `review-blocking` with the worktree clean, the entire fix already committed as `2973a5ca`, and the blocking finding a doc-surfacing gap. <!-- cite: FX.13 incident:K#1988 -->

  - **PRE-acceptance kinds — `blocked` / `design-fork` / `failed` / a machinery escalation / `acceptance-incomplete`** (anything raised *before* 3e.5 returned a passing verdict, so little or no accepted work exists and re-asking is cheap) → **park + stop** via the park path below: a terminal **parked** disposition for this single-item run, and resume = re-run `/fix` on the same target after answering, **never an in-place continuation for these kinds** (the park path removes the worktree, so there is deliberately nothing left to resume against).
  - **POST-acceptance kinds — `review-blocking`, and any other kind raised after the acceptance gate has already passed** → **resume in place**, on the continuation path `build-level.mjs` already supports natively and `/build` 3d-esc already drives for this same escalation kind. Surface the findings via `decision_sink_ask(<the payload's findings + the proposed disposition>, [fix the findings (continue in place), park it for a human, …], ask-now)`; on **fix**, re-invoke the **same** Workflow with the Step 4a `args` block unchanged except for these two keys: <!-- cite: FX.14 class:finished-build-discarded -->
...
- **Parked (open question / not-ready / held)** — `#<issue>` → the reason it parked (the worker's question, `reattach`'s not-ready reason, or a declined merge), and which marker carries it forward (`needs-clarification` for a question; the open PR for a held/not-ready adopt) so a re-run resumes it. A **post-acceptance** escalation (`review-blocking`) is **not** parked by default — 4a resumes it in place in the same run — so it reaches this bullet only when the operator explicitly chose park, and then the park path released its worktree and claim like any other parked target.
