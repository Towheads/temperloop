...
- **Every run ends at a terminal disposition — never at "PR opened."** The terminal set is: **merged** (code, PR confirmed `MERGED` + issue closed), **resolved (verdict)** (a spike, verdict-closed with a comment), **parked** (an open question / not-ready adoption / a held merge, recorded for a human), or **reported-no-op** (already-done / claimed-elsewhere / not-found / probe-failed / not-an-issue / epic-refused / blocked-stopped). A `/fix` run that stops at "opened a PR" is a bug — the merge gate (Step 5) is part of the same run. <!-- cite: FX.4 class:abandoned-ownerless-prs -->
...
The target is drivable. Claim is the **first mutating action** (owned by `build-level.mjs` 3a). **Immediately before invoking**, print the **Workflow launch line** template (`claude/message-schema.md`) — `caller: fix`, this being a 1-item level, `round 1` for this fresh drive (a `review-blocking` escalation re-invokes the same Workflow at `round 2`+ as a 3d-esc-shaped continuation — see Step 4a's escalation branch below; a park-and-discard kind parks instead, so it never mints a round 2) — so a `/fix` run is distinguishable in the transcript from any concurrent `/build`/`/sweep` drive on the same repo (temperloop#903, temperloop#1941). Invoke the saved Workflow as a **1-item level**, exactly as `/sweep` Phase 2 does (invoke by `scriptPath`; `args` delivered as a JSON string the script parses):
...
Workflow({ scriptPath: workflowPath, args: {
  ...
  verdicts: {}, onlySlugs: []
                         ...
                         // A `review-blocking` escalation (4a's continuation arm) re-invokes this
                         // same args block with BOTH keys populated; that is what makes
                         // driveItem() take isContinuation and resume at 3c against the intact
                         // worktree instead of re-creating (and discarding) it.
} })
```
...
- **`escalations: [{ slug, kind, payload }]`** → **branch on whether a committed build already exists to resume against** — that, not a blanket rule, is what is at stake. A **blanket** park-and-discard rule is wrong because it throws away work already earned — the commit sitting in the worktree — to re-earn it from scratch for what is often a few lines. temperloop#1988 (resume-rule review blocking)'s live case is the evidence: `/fix temperloop#1970` (reviewer serial-HIGH loop) escalated `review-blocking` with the worktree clean, the entire fix already committed as `2973a5ca`, and the blocking finding a doc-surfacing gap. <!-- cite: FX.13 incident:K#1988 -->

  - **Park-and-discard kinds — `blocked` / `design-fork` / `failed` / a machinery escalation / `acceptance-incomplete`** (each raised with little or no committed work behind it, so re-asking is cheap) → **park + stop** via the park path below: a terminal **parked** disposition for this single-item run, and resume = re-run `/fix` on the same target after answering, **never an in-place continuation for these kinds** (the park path removes the worktree, so there is deliberately nothing left to resume against).
  - **`review-blocking`, at EITHER of its two raise sites** → **resume in place**, on the continuation path `build-level.mjs` already supports natively and `/build` 3d-esc already drives for this same escalation kind. Surface the findings via `decision_sink_ask(<the payload's findings + the proposed disposition>, [fix the findings (continue in place), park it for a human, …], ask-now)`; on **fix**, re-invoke the **same** Workflow with the Step 4a `args` block unchanged except for these two keys: <!-- cite: FX.14 class:finished-build-discarded -->

    ```
    verdicts: { "<slug>": { kind: "<the escalation kind>", verdict_section: "<the FULL markdown block — heading + body — carrying the findings VERBATIM plus the operator's disposition>" } },
    onlySlugs: ["<slug>"]
    ```

    **Which side of the acceptance gate it came from is READ OFF THE PAYLOAD, never assumed from the kind.** `build-level.mjs` raises `review-blocking` from **two** sites, and only one of them is post-acceptance:

    | payload | raised at | vs. the 3e.5 acceptance gate |
    |---|---|---|
    | no `stage` key | §3e **pre-push reviewer pass** | **BEFORE** — 3e.5 has not run yet |
    | `stage: 'ci-fix'` | §3g **CI-fix re-review**, after the PR is open | **AFTER** — 3e.5 passed, and so did the first CI round |

    **The behavior below is identical for both** — keep the worktree, keep the claim, resume at 3c — because at both sites a real committed diff exists, and destroying it is precisely the loss this arm prevents. What the marker changes is only **what has already been verified**: a payload with no `stage` still has 3e.5 ahead of it, so never read "resume in place" as "acceptance is settled, skip re-checking it". (Even a `stage: 'ci-fix'` continuation re-runs 3e.5 against the new commit — the continuation resumes at 3c and goes forward through the full 3e/3e.5/3f sequence either way.)

    `driveItem()` reads `onlySlugs.includes(<slug>)` as `isContinuation` and therefore **skips 3a** (this run still holds the claim) and **skips 3b** — `worktree.sh create` force-removes-and-re-adds, which would discard the committed build — resuming at **3c** against the intact worktree with `verdict_section` injected verbatim into the re-spawned worker's prompt, then forward through 3e/3e.5/3f as normal (`pr.sh open` returns `EXISTS` and adopts an already-open PR, so a post-push `review-blocking` from 3g's CI-fix re-review continues safely too). **The park path below is SKIPPED for this arm**: the item **keeps its worktree and its claim**, which are precisely what the continuation resumes against. Print the **Workflow launch line** with the **incremented `round`** and the escalation kind before the re-invocation and the return line after, so a continuation round is never indistinguishable from the fresh drive that preceded it (temperloop#1941). If the operator chooses **park** instead, fall through to the park path.

  **The park path** — taken by every park-and-discard kind above, and by a `review-blocking` escalation only when the operator explicitly parks it. Surface the escalation via `decision_sink_ask(<the payload's question / design_fork.decision+options / failure_reason>, <per-kind options>, ask-now)`; if the operator answers **inline**, record the answer and **re-run the drive from Step 2** (the answer rides into 4a's item `notes:`). Otherwise **park the target so a later re-run is lossless** — mirror `/sweep`'s escalation park exactly (**never leave the claim held or the worktree as debris** — a still-held claim makes a same-session re-run route back through `fresh` and `worktree.sh create` **force-clear** the orphaned worktree, silently discarding the escalated worker's uncommitted edits; a cross-session re-run instead reads `claimed-elsewhere` and 4d stops, blaming a dead prior run): <!-- cite: FX.8 class:escalated-work-destruction -->
  ...
  4. `"$RELEASE" <issue> --board "$BOARD"` (clear the claim marker), then `workflows/scripts/build/worktree.sh remove "$repoRoot" <slug>` — the workflow leaves an escalated worktree **intact**, and for the kinds that park here `/fix` discards it (resume = re-run, not an in-place continuation), so remove it now. This is what prevents the next run's `worktree.sh create` from force-clearing an orphaned worktree — the lossless-re-run guarantee, which still holds unchanged for every kind that reaches this step. **Never run this step for a `review-blocking` escalation taking the continuation arm above** (at either raise site): its worktree holds a finished, committed build and its claim is what the continuation resumes under, so releasing either destroys exactly the work that branch exists to preserve.
...
- **Parked (open question / not-ready / held)** — `#<issue>` → the reason it parked (the worker's question, `reattach`'s not-ready reason, or a declined merge), and which marker carries it forward (`needs-clarification` for a question; the open PR for a held/not-ready adopt) so a re-run resumes it. A `review-blocking` escalation is **not** parked by default — 4a resumes it in place in the same run, at either of its two raise sites — so it reaches this bullet only when the operator explicitly chose park, and then the park path released its worktree and claim like any other parked target.
