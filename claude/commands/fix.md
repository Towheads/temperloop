...
- **`escalations: [{ slug, kind, payload }]`** (the worker hit a question / blocker — `blocked` / `design-fork` / `failed` / a machinery escalation) → **park + stop** (a terminal **parked** disposition for this single-item run; `/fix` drives *one* item, so a worker escalation ends the run — resume = re-run `/fix` on the same target after answering, **never an in-place continuation**, since `/fix` always invokes `build-level.mjs` with `verdicts: {}, onlySlugs: []`, i.e. never `isContinuation`). Surface the escalation via `decision_sink_ask(<the payload's question / design_fork.decision+options / failure_reason>, <per-kind options>, ask-now)`, then take one of the two branches below.
  ...
  **What the next `worktree.sh create` destroys — this governs BOTH branches.** `build-level.mjs` leaves an escalated worktree **intact**, and any later drive of the same slug calls `worktree.sh create`, whose first act is `clear_path`: it force-removes `<repoRoot>.wt/<slug>` **and `git branch -D`s the local branch `build/<slug>`**, then re-adds that branch off a freshly-fetched `origin/<default>`. So it discards the escalated worker's uncommitted edits **and every commit it made** — committing is *not* protection, and no other ref in that same repo (a tag, a stash, a second local branch) survives either, because `clear_path` operates on that repo. Only a ref **outside** the local repo does. <!-- cite: FX.8 class:escalated-work-destruction -->
  ...
  - **Operator answers inline** → **preserve the escalated attempt's work FIRST, then re-drive.** The operator being present is exactly when real work exists on disk, so this branch — not the park branch — is the common one:
    1. **Probe what is there** — `pr.sh recover-probe "<repoRoot>.wt/<slug>" "build/<slug>"`. Read `.ahead` (commits on the escalated branch) and `.dirty` / `.dirty_files`. `ahead: 0` with `dirty: false` → there is nothing to preserve; skip to item 4.
...
1. **Reclaim the worktree (in-lane, idempotent)** — for a **merged** or **resolved (verdict)** disposition that built in a worktree (the 4a fresh drive; not the 4b adopt path, which owns no local worktree): `workflows/scripts/build/worktree.sh remove "$repoRoot" <slug>`. Idempotent by construction — `cmd_remove` returns `REMOVED` when a worktree/`build/<slug>` branch existed and `NOT_FOUND` otherwise, so a re-run (or the deploy-mini session-start sweep, F#653, the crash-path backstop) is a safe no-op.
   ...
   **Delete the preservation ref — but ONLY after proving the work it holds actually landed.** If this run pushed `build/<slug>` to origin (4a's inline-answer escalation branch item 3, or its park branch step 1), that ref may be the last copy of reviewed work, so its deletion is **gated on a mechanical ancestry check, never on the disposition alone**. A merged disposition only proves *a* PR merged; it does **not** prove the re-drive's worker actually performed the `git merge --ff-only` restore that 4a's `notes:` instructed. That instruction is free-text prose handed to a model, and silent plan drift is a known failure mode — a worker that skipped it still produces a green, merged PR, so deleting on the merge alone would destroy the reviewed commit *silently and unrecoverably*. Gate the irreversible step instead:
   ...
   `<preserved-sha>` is the `.sha` the push recorded (4a item 3; re-readable via `git ls-remote origin "refs/heads/build/<slug>"`). The test asks the only question that matters: **is the preserved commit an ancestor of the merged default tip?**
   ...
   **On a parked or held disposition the ref is deliberately KEPT** — and that is not a leak: the park path removed the worktree and with it the local `build/<slug>`, so the pushed ref is then the *only* surviving copy of the escalated work. The park comment (4a park step 3) names it, and the next `/fix` run on this issue deletes it here, through this same gate, at *its* terminal disposition.
