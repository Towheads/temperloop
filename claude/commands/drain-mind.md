...
- Vault **reads** (including `search_vault_smart` below) go through the Obsidian MCP, the agent-plane transport per the contract's Obsidian-mode note. Vault **writes** performed by the extraction/learning steps below (Decisions/Patterns/Mistakes/Context notes, the candidate-tells and friction ledgers, and the pending-decisions surface) are **direct** file writes (`Write`/`Edit`/Bash append) under the knowledge-store root *(writer migrated per F#953 — writes are direct; reads remain on MCP)*; the drain lock file and stub list/delete operations are unaffected and remain on the Obsidian MCP pending a later migration phase. Use the Things MCP for task creation.
...
- If missing, write `Decisions/<project> - <short title>.md` directly with the `Write` tool *(writer migrated per F#953 — writes are direct; reads remain per the store-access rule)*, using the vault provenance schema frontmatter (`tags: [decision, project/<name>]`) and body covering: **what** was decided, **why**, **alternatives considered**, **trade-off accepted**. Cross-link to superseded decisions via `[[wikilinks]]`. End with the `## Source` footer.
...
- **Clear authorship** — the stub contains assistant-turn phrases like "wrote `Decisions/X.md`", "captured to vault", "writing the Decision now", "I'll create the decision file", or a tool-call result showing the file was created. Backfill in place with a direct `Edit` on the note file *(writer migrated per F#953 — writes are direct; reads remain per the store-access rule)*:
...
### Patterns
Reusable approaches that worked. Save directly to vault `Patterns/<title>.md` with the `Write` tool *(writer migrated per F#953 — writes are direct; reads remain per the store-access rule)*, using the vault provenance schema frontmatter (`tags: [pattern, project/<name>, ...]`) and the `## Source` footer. Skip if a pattern with the same title already exists.

### Mistakes
Pitfalls, failure modes, things that broke. Save directly to vault `Mistakes/<title>.md` with the `Write` tool *(writer migrated per F#953 — writes are direct; reads remain per the store-access rule)*, using the vault provenance schema frontmatter (`tags: [mistake, project/<name>]`) and the `## Source` footer. Skip duplicates.
...
For each such note, apply the stranger test: would a stranger's kernel-only install need this for the kernel machinery (board adapter, build/sweep pipeline, install/doctor, branch/PR policy) to work correctly? If yes and the note isn't already tagged `kernel-candidate`, add that tag to its frontmatter `tags:` list with a direct `Edit` (a targeted `tags:` field edit, not a rewrite) *(writer migrated per F#953 — writes are direct; reads remain per the store-access rule)* — this flags it for eventual upstream contribution once the kernel repo exists as a live checkout. Never remove an existing `kernel-candidate` tag, and never tag a note the stranger test doesn't clearly pass — **default to `overlay`** (no tag), matching `/triage`'s Step 2.8 default (a missed kernel tag costs nothing; a wrongly-added one misroutes a personal/org-specific note).
...
2. **Backfill the missing entry.** If absent, append one `### open` entry in the note's format (Decision + Default taken + Disposition + Status: open) directly — a Bash `>>` append *(writer migrated per F#953 — writes are direct; reads remain per the store-access rule)* — with Disposition noting it was backfilled by `/drain-mind` from `Sessions/<stub>`. This re-arms the `plan-morning` review the live write would have triggered.
...
If the vault file does not yet exist, create it first directly with the `Write` tool *(writer migrated per F#953 — writes are direct; reads remain per the store-access rule)*, with this header:
