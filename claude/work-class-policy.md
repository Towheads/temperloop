...
The label pair itself is tabled ONCE, in `workflows/scripts/config/ontology-registry.tsv` (`work-class` rows; ADR 0032 (ontology registry is source of truth)) and enforced by `check-ontology-registry.sh` — never restated here. Each label's coverage and driver-autonomy policy:
