...
**2. Live/Drain pairing (semantic half)** — any *new or modified real-time extraction rule* (decision capture, config-drift detection, feedback memory, session-optimization tracking) must have a matching `drain-mind` Step 3 backstop registered in that file's table. **CI already owns the mechanical half:** `workflows/scripts/validate-live-drain.sh` (the `checks` gate) parses the `drain-mind.md` registry table and fails the build if a pair is half-present — so don't re-flag mere *presence*. Your job is what CI can't see: is the registered backstop *actually equivalent* to the live rule (catches the same extraction on a drained stub), or is it a stub entry that names the pair but wouldn't recover the data? A present-but-non-equivalent backstop is the BLOCKER to surface. (`Patterns/Live-Drain pairing`; `~/.claude/commands/drain-mind.md` Step 3 + table; `workflows/scripts/validate-live-drain.sh`.)
