- **New `PreToolUse` guard `claude/hooks/arm-read-guard.sh` enforces dual-build
  ...
  arm's worktree path or branch is **denied**, and the attempt is appended to
  `.dual-build-cross-read-attempts.jsonl` beside the marker for the build
  ledger's `cross_read_attempted` field. Without that marker the hook has no
  effect at all, and any internal error fails open, so a session that is not an
  arm of a dual build is never affected. Registering it is opt-in: add it to
