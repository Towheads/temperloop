- **A gate-registration-only `scripts/quality-gates.sh` diff no longer escalates
  ...
  as a registration. Every other edit to that file keeps the full escalation, as
  does a diff the probe cannot read; the run's reason line names the exception
  whenever it is taken. The probe resolves its own diff base when the caller
  supplies none, so the exception holds on every slice of a sliced `--scoped`
  run rather than only the first, and it pins its own diff prefixes so a
  `diff.mnemonicPrefix` / `diff.noprefix` / `diff.external` git config cannot
  silently disable it.
