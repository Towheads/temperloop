...
def validate_line(line):
    ...
    # FIXTURE RESIDUE keeps its own verdict and remedy (temperloop#1747).
    # HOISTED above the shape check by temperloop#1756's structural rewrite: the
    # old family enum rejected 'recorded-stub-model' on its missing `claude-`
    # prefix, so the residue branch sat inside that failure path. The structural
    # regex ACCEPTS it (lowercase segments joined by -), so leaving the check
    # nested there would have silently retired the #1747 guard.
