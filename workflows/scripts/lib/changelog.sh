...
#     content BYTE-PRESERVED — every pre-existing line of the body, blank
#     lines included and however many consecutive ones there are, is re-emitted
#     verbatim. The rewrite is INSERT-ONLY: the only lines that may differ from
#     the input are the ones being added (plus the ` — BREAKING` suffix edit
#     below, which is a deliberate content change, not whitespace churn);
...
#
# WHY BYTE-PRESERVATION IS THE CONTRACT, not a nicety. The emitter used to
# RECONSTRUCT the body — stripping each sub-section's leading/trailing blank
# run and re-emitting one canonical blank line around every heading. That made
# the output depend on incidental whitespace in the INPUT: a `main` whose
# `[Unreleased]` happened to carry a stray double blank line assembled to one
# line FEWER than it started with, while a `main` without one assembled purely
# additively. The same code, two answers, differing only in a blank line —
# which is how a green PR was ejected from the merge queue when the trial
# branch merged in a `main` that had picked one up (temperloop#1321). Since
# CHANGELOG.md is a file every PR touches, an input-dependent rewrite keeps
# manufacturing surprise diffs. So the emitter below is INSERT-ONLY: it walks
# the body and re-emits every line verbatim, splicing the new content in at
# computed offsets. Nothing is ever normalised, reflowed, or dropped.
# ---------------------------------------------------------------------------
...
_changelog_assemble_awk() {
  awk '
    ...
    # Emit the BRAND-NEW category sections (ones the body has no sub-heading
    # for) parked in slot `k` by the planning pass, so a new section lands at
    # its Keep-a-Changelog position instead of being dumped at the end.
    #
    # Each block is self-delimiting: a leading blank ONLY when the output does
    # not already end in one, and always a trailing blank. Both are ADDED
    # lines, never edits of existing ones, so this stays insert-only — and the
    # trailing blank is what separates the new section from whatever verbatim
    # line comes next (typically the existing heading this slot sits before).
    function flush_new_at(k,   m, arr, j, c, h) {
      m = split(NEWAT[k], arr, " ")
      for (j = 1; j <= m; j++) {
        c = arr[j]
        if (c == "") continue
    ...
    END {
      ...
      for (k = 1; k <= nh; k++) {
        HEADIDX[H[k]] = k
        if (HC[k] != "" && !EXISTS[HC[k]]) EXISTS[HC[k]] = k
      }
      ...
      # ---- PLANNING PASS ----------------------------------------------------
      # Nothing is emitted here; this only computes WHERE the new content is
      # spliced into the verbatim line stream, so the emission pass below can
      # stay a straight insert-only walk of L[1..n].
      #
      # APPEND[i] — the category whose fragment block is inserted immediately
      # AFTER line i. For a sub-section that already exists, that anchor is its
      # LAST NON-BLANK line, so the block lands at the end of that content and
      # the trailing blank run belonging to the section is emitted afterwards,
      # untouched. A sub-section with no content at all anchors on its heading.
      #
      # (No apostrophes in this comment block on purpose: the whole awk program
      # is a single-quoted shell word, and one would close the quote.)
      for (k = 1; k <= nh; k++) {
        c = HC[k]
        if (c == "" || !HAVE[c] || DONE[c]) continue
        DONE[c] = 1
        TAKE[k] = c
        bs = H[k] + 1
        be = (k < nh) ? H[k + 1] - 1 : n
        while (be >= bs && L[be] ~ /^[ \t]*$/) be--
        APPEND[(be >= bs) ? be : H[k]] = c
      }
      # NEWAT[k] — brand-new sections (no sub-heading in the body) to flush
      # just BEFORE heading k; slot nh+1 is the end-of-body flush. EXISTS[]
      # gates it: a category that DOES have a sub-heading is merged there by
      # the APPEND pass above, never duplicated as a new section here. The
      # canonical `j` order is preserved within a slot.
      for (j = 1; j <= ncat; j++) {
        c = CATS[j]
        if (!HAVE[c] || EXISTS[c]) continue
        slot = nh + 1
        for (k = 1; k <= nh; k++) if (ER[k] > RANK[c]) { slot = k; break }
        NEWAT[slot] = NEWAT[slot] c " "
      }

      # ---- EMISSION PASS ----------------------------------------------------
      # INSERT-ONLY. Every L[i] goes out verbatim — blank runs, indentation and
      # all — and the only other lines emitted are the new blocks planned
      # above. The single exception is the ` — BREAKING` suffix on a heading a
      # breaking fragment merged into, which is a deliberate content edit.
      for (i = 1; i <= n; i++) {
        k = HEADIDX[i]
        if (k) {
          if (NEWAT[k] != "") flush_new_at(k)
          head = L[i]
          c = TAKE[k]
          if (c != "" && BR[c] == 1 && head !~ /BREAKING/) head = "### " T[c] SUFFIX
          emit(head)
        } else {
          emit(L[i])
        }
        if (APPEND[i] != "") {
          emit("")
          emit_block(ADD[APPEND[i]])
          # Only fires when the next line is a heading that already butted
          # straight up against the content with no blank between — a blank we
          # ADD rather than one we removed, so every input line survives.
          if (i < n && L[i + 1] !~ /^[ \t]*$/) emit("")
        }
      }
      if (NEWAT[nh + 1] != "") flush_new_at(nh + 1)
      # The body must end on a blank line so the next `## [` heading is not
      # glued to the last entry. Appended, never substituted for a line.
      if (on == 0 || OUT[on] != "") emit("")
