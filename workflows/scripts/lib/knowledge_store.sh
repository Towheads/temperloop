...
# Default-root probe for the temperloop#165 rename window (v0.14.0,
...
# The legacy fallback is removed in v0.16.0 (VERSIONING.md pre-1.0 window;
# the v0.14.0 CHANGELOG BREAKING entry carries the migration note).
...
# simulates the post-v0.16.0 removal — the legacy store is then named
...
_ks_default_root() {
  ...
  if [ ! -d "$new_root" ] && [ -d "$old_root" ]; then
    if [ "${TEMPERLOOP_LEGACY_WINDOW_CLOSED:-0}" = "1" ]; then # knob:exempt — test/simulation-only seam
      printf 'knowledge_store: NOTE — a legacy store exists at %s but the legacy default-root fallback was removed in v0.16.0; the default root is now %s. Move the store (mv "%s" "%s") or set KNOWLEDGE_STORE_ROOT.\n' \
        "$old_root" "$new_root" "$old_root" "$new_root" >&2
    else
      printf 'knowledge_store: NOTE — using legacy store root %s (default moved to %s in v0.14.0; legacy fallback removed in v0.16.0 — move the store or set KNOWLEDGE_STORE_ROOT).\n' \
        "$old_root" "$new_root" >&2
      printf '%s\n' "$old_root"
      return 0
    fi
