#!/usr/bin/env bash
# check-knob-registry.sh — forwarding stub (terminology consolidation v0.17.0,
# temperloop#729). Renamed to check-setting-registry.sh; this stub forwards through
...
printf 'NOTE: %s was renamed %s in v0.17.0 (temperloop#729); this forwarding stub is removed in v0.19.0.\n' "check-knob-registry.sh" "check-setting-registry.sh" >&2
exec "$(cd -P "$(dirname "${BASH_SOURCE[0]}")" && pwd)/check-setting-registry.sh" "$@"
