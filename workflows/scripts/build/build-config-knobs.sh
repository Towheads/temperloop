#!/usr/bin/env bash
# build-config-knobs.sh — forwarding stub (terminology consolidation v0.17.0,
# temperloop#729). Renamed to build-config-settings.sh; this stub forwards through
...
printf 'NOTE: %s was renamed %s in v0.17.0 (temperloop#729); this forwarding stub is removed in v0.19.0.\n' "build-config-knobs.sh" "build-config-settings.sh" >&2
exec "$(cd -P "$(dirname "${BASH_SOURCE[0]}")" && pwd)/build-config-settings.sh" "$@"
