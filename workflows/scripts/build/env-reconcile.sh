...
#   - The sync stamps a provenance banner onto its output ("# GENERATED by
#     foundation '<cmd>' …" / "# Source of truth: …"). Those banner lines are
#     EXCLUDED from the comparison — they are present by construction in every
#     synced copy and absent from the canonical file, so counting them would
#     make every consumer permanently "drifted".
#   - The remedy is read from the copy's OWN banner (which names the exact sync
#     command that produced it), falling back to the generic engine invocation.
#     No mapping table to keep in sync with the Makefile's wrappers.
#
...
# _hook_body <file> — the file's content with the sync's provenance BANNER lines
# removed, so a synced copy and its canonical source compare equal when only the
# banner differs. Two banner shapes are stripped, exactly the pair the hook-sync
# injects: the "GENERATED by … DO NOT EDIT HERE" line and its "Source of truth:"
# follow-on. Nothing else is normalised — any other byte difference IS drift.
_hook_body() {
  sed -e '/^#[[:space:]]*GENERATED by /d' -e '/^#[[:space:]]*Source of truth:/d' "$1" 2>/dev/null
}
...
# that could itself drift from the Makefile's wrappers.
        ...
        while [ "$_j" -lt "${#VENDORED_HOOKS[@]}" ]; do
          ...
          case "$cls" in
            *"STALE_VENDORED_HOOK:$h"*)
              ...
              OPERATOR_LINES="${OPERATOR_LINES}               ↳ re-sync $h: $remedy"$'\n'
              add "  - remedy — re-sync $h into $c: \`$remedy\`"
              ;;
          esac
        done
        ;;
