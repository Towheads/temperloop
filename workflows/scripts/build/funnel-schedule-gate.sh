#!/usr/bin/env bash
# funnel-schedule-gate.sh — forwarding stub (terminology consolidation v0.17.0,
# temperloop#729). Renamed to pipeline-schedule-gate.sh; this stub forwards through
...
printf 'NOTE: %s was renamed %s in v0.17.0 (temperloop#729); this forwarding stub is removed in v0.19.0.\n' "funnel-schedule-gate.sh" "pipeline-schedule-gate.sh" >&2
exec "$(cd -P "$(dirname "${BASH_SOURCE[0]}")" && pwd)/pipeline-schedule-gate.sh" "$@"
