#!/usr/bin/env bash
#
# pipeline-overlap.sh — /build's run-start pipeline-interference predicate
# (foundation #864, from the Epic B retro #847). A standalone, testable
# predicate: does this plan's aggregate file set intersect the pipeline's
# operational surface while the pipeline is enabled for this repo's board?
# When it does, /build Step 1.7 surfaces a freeze offer AT RUN START —
# before the run and the pipeline spend hours rebasing over each other
# (the Epic B cascade: repeated re-gate cycles over moved main + a stale
# kernel seed → design fork → ~2.5h of rework).
#
#   pipeline-overlap.sh [--board N] <file...>     # plan items' files: entries
#
...
# the pipeline's effective board set (schedule `boards:` falling back to
# PIPELINE_ENABLED_BOARDS), and ≥1 given file starts with a PIPELINE_DRIVEN_PATHS
# prefix. Everything else is a no-overlap.
#
# FAIL-OPEN is the load-bearing invariant — the deliberate INVERSE of
# pipeline-schedule-gate.sh's fail-closed. That gate guards spend, so it defaults
...
# jq, an absent --board (repo not board-wired → not pipeline-driven), or an empty
...
#   PIPELINE_SCHEDULE_FILE   the operator-controls schedule note (same
#                          resolution as pipeline-schedule-gate.sh — ks_root,
...
#   PIPELINE_ENABLED_BOARDS  default board set when the schedule's `boards:` is
#                          empty (same fallback pipeline-cron.sh uses)
#   PIPELINE_DRIVEN_PATHS    space-separated path prefixes = the pipeline's
...
# Same schedule-note resolution as pipeline-schedule-gate.sh (ks_root seam) —
...
# non-vendoring-checkout fallback, per setting-registry.tsv's own convention) —
# keep both in sync.
...
# Same non-vendoring fallbacks the other pipeline consumers keep.
...
[ -n "$board" ] || no_overlap "no --board -- repo is not board-wired, so not pipeline-driven"
[ -r "$PIPELINE_SCHEDULE_FILE" ] || no_overlap "schedule file missing/unreadable (pipeline fail-closed quiet): $PIPELINE_SCHEDULE_FILE"

# Extract the fenced ```pipeline-schedule … ``` block body — same awk idiom as
# pipeline-schedule-gate.sh; only the FIRST block is read.
block="$(awk '/^```pipeline-schedule[[:space:]]*$/{f=1;next} f&&/^```/{exit} f' "$PIPELINE_SCHEDULE_FILE" 2>/dev/null || true)"
[ -n "$block" ] || no_overlap "no pipeline-schedule block found (pipeline fail-closed quiet)"

# Pull one `key:` value out of the block — verbatim pipeline-schedule-gate.sh
...
case "$enabled" in
  ...
  *) no_overlap "pipeline disabled/frozen (enabled: '${enabled:-absent}')" ;;
esac

# Effective board set: schedule `boards:` override, else the code default —
# the same precedence pipeline-cron.sh applies.
sched_boards="$(field boards)"
eff_boards="${sched_boards:-$PIPELINE_ENABLED_BOARDS}"
in_boards=0
for b in $eff_boards; do
  [ "$b" = "$board" ] && in_boards=1
done
[ "$in_boards" -eq 1 ] || no_overlap "board $board not in pipeline boards ($eff_boards)"

# Prefix-intersect the plan's file set against the pipeline's operational surface.
matched=()
for f in "${files[@]}"; do
  for p in $PIPELINE_DRIVEN_PATHS; do
    case "$f" in
      "$p"*) matched+=("$f"); break ;;
    esac
  done
done
[ "${#matched[@]}" -gt 0 ] || no_overlap "no plan file under PIPELINE_DRIVEN_PATHS"
