#!/usr/bin/env bash
#
# Tests for workflows/scripts/build/pipeline-overlap.sh — /build's run-start
# pipeline-interference predicate (foundation #864).
#
...
# PIPELINE_SCHEDULE_FILE at it, pins the board set / driven-paths via env, runs
...
# no-overlap (fail-open) · empty boards: falls back to PIPELINE_ENABLED_BOARDS
# · no files → inconclusive no-overlap · no --board → no-overlap ·
# PIPELINE_DRIVEN_PATHS override honored · malformed enabled token fails open ·
# (temperloop#226/#232) PIPELINE_SCHEDULE_FILE's UNSET-default resolution —
# Controls/ then legacy Context/ fallback, same probe as pipeline-schedule-gate.sh.

set -euo pipefail

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PRED="$HERE/../pipeline-overlap.sh"
...
run() {
  ...
  shift
  env "${envs[@]}" PIPELINE_SCHEDULE_FILE="$sched" bash "$PRED" "$@"
}
...
rc=0; v=$(run "$TMP/on.md" PIPELINE_DRIVEN_PATHS="workflows/scripts/ Makefile" -- \
...
# ── 2: board not in the pipeline's board set → no-overlap ──────────────────────
echo "--- test 2: board ∉ boards → no-overlap ---"
write_sched "$TMP/b3.md" "enabled: yes" "hours: 9" "boards: 3"
rc=0; v=$(run "$TMP/b3.md" PIPELINE_DRIVEN_PATHS="workflows/scripts/" -- \
...
rc=0; v=$(run "$TMP/off.md" PIPELINE_DRIVEN_PATHS="workflows/scripts/" -- \
      --board 4 workflows/scripts/board/lib/board.sh) || rc=$?
...
rc=0; v=$(run "$TMP/nope.md" PIPELINE_DRIVEN_PATHS="workflows/scripts/" -- \
      --board 4 workflows/scripts/board/lib/board.sh) || rc=$?
[ "$rc" = "0" ] && ok "exit 0" || bad "missing.rc" "got $rc"
[ "$(field "$v" .action)" = "no-overlap" ] && ok "action=no-overlap" || bad "missing.action" "got $v"

# ── 5: note without a pipeline-schedule block → no-overlap (fail-open) ─────────
echo "--- test 5: no fenced block → no-overlap ---"
echo "just prose, no block" > "$TMP/prose.md"
rc=0; v=$(run "$TMP/prose.md" PIPELINE_DRIVEN_PATHS="workflows/scripts/" -- \
      --board 4 workflows/scripts/board/lib/board.sh) || rc=$?
[ "$rc" = "0" ] && ok "exit 0" || bad "noblock.rc" "got $rc"

# ── 6: empty boards: falls back to PIPELINE_ENABLED_BOARDS ─────────────────────
echo "--- test 6: boards: absent → PIPELINE_ENABLED_BOARDS fallback ---"
write_sched "$TMP/nb.md" "enabled: yes" "hours: 9"
rc=0; v=$(run "$TMP/nb.md" PIPELINE_ENABLED_BOARDS="4" PIPELINE_DRIVEN_PATHS="workflows/scripts/" -- \
      --board 4 workflows/scripts/board/lib/board.sh) || rc=$?
[ "$rc" = "10" ] && ok "fallback board matches → exit 10" || bad "fallback.rc" "got $rc"
rc=0; v=$(run "$TMP/nb.md" PIPELINE_ENABLED_BOARDS="3" PIPELINE_DRIVEN_PATHS="workflows/scripts/" -- \
      --board 4 workflows/scripts/board/lib/board.sh) || rc=$?
...
rc=0; v=$(run "$TMP/on2.md" PIPELINE_DRIVEN_PATHS="workflows/scripts/" -- --board 4) || rc=$?
...
# ── 8: no --board → no-overlap (repo not pipeline-driven) ──────────────────────
echo "--- test 8: no --board → no-overlap ---"
rc=0; v=$(run "$TMP/on2.md" PIPELINE_DRIVEN_PATHS="workflows/scripts/" -- \
...
rc=0; v=$(run "$TMP/on2.md" PIPELINE_DRIVEN_PATHS="workflows/scripts/ Makefile" -- \
      --board 4 dashboard/index.html env/zshrc) || rc=$?
[ "$rc" = "0" ] && ok "exit 0" || bad "disjoint.rc" "got $rc"
case "$(field "$v" .reason)" in *PIPELINE_DRIVEN_PATHS*) ok "reason names the prefix set" ;; *) bad "disjoint.reason" "got $v" ;; esac

# ── 10: PIPELINE_DRIVEN_PATHS override honored ─────────────────────────────────
echo "--- test 10: driven-paths override ---"
rc=0; v=$(run "$TMP/on2.md" PIPELINE_DRIVEN_PATHS="dashboard/" -- --board 4 dashboard/index.html) || rc=$?
...
rc=0; v=$(run "$TMP/bad.md" PIPELINE_DRIVEN_PATHS="workflows/scripts/" -- \
      --board 4 workflows/scripts/board/lib/board.sh) || rc=$?
[ "$rc" = "0" ] && ok "exit 0" || bad "malformed.rc" "got $rc"

# ── 12: PIPELINE_SCHEDULE_FILE default resolution — Controls/ then Context/
#    fallback (temperloop#226/#232). Leaves PIPELINE_SCHEDULE_FILE UNSET and
...
#    PIPELINE_SCHEDULE_FILE explicitly and never exercise it).
echo "--- test 12: PIPELINE_SCHEDULE_FILE default — Controls/ then Context/ fallback ---"
KROOT="$TMP/kstore"
mkdir -p "$KROOT/Controls" "$KROOT/Context"

# 12a: Controls/ file present (Context/ absent) → overlap fires from Controls/.
write_sched "$KROOT/Controls/foundation - pipeline schedule.md" "enabled: yes" "hours: 9" "boards: 4"
rc=0; v=$(env KNOWLEDGE_STORE_ROOT="$KROOT" PIPELINE_DRIVEN_PATHS="workflows/scripts/" \
...
write_sched "$KROOT/Context/foundation - pipeline schedule.md" "enabled: yes" "hours: 9" "boards: 4"
rc=0; v=$(env KNOWLEDGE_STORE_ROOT="$KROOT" PIPELINE_DRIVEN_PATHS="workflows/scripts/" \
      bash "$PRED" --board 4 workflows/scripts/board/lib/board.sh) || rc=$?
[ "$rc" = "10" ] && ok "overlap fires reading the legacy Context/ path only" || bad "default12b.rc" "got $rc"
rm -f "$KROOT/Context/foundation - pipeline schedule.md"
