#!/usr/bin/env bash
#
# Tests for the autonomous pipeline driver's CRON layer (foundation #596):
#   pipeline-schedule-gate.sh — the operator-editable, fail-closed schedule gate.
#   pipeline-cron.sh          — the gate → tick(emit) → log/notify wrapper.
#
# Like test_pipeline_tick.sh, these run entirely OFFLINE: the gate's hour and
# schedule-file are injected (PIPELINE_NOW_HOUR / PIPELINE_SCHEDULE_FILE) so there is
...
# is stubbed (PIPELINE_NOTIFY_CMD) so nothing is actually displayed.
#
...
GATE="$HERE/../pipeline-schedule-gate.sh"
CRON="$HERE/../pipeline-cron.sh"
...
F="$TMP/sched1.md"
printf '%s\n' '# pipeline schedule' '' '```pipeline-schedule' 'enabled: yes' 'hours: 9 14 20' '```' > "$F"
run_gate env PIPELINE_NOW_HOUR=14 PIPELINE_SCHEDULE_FILE="$F" bash "$GATE"
...
run_gate env PIPELINE_NOW_HOUR=15 PIPELINE_SCHEDULE_FILE="$F" bash "$GATE"
...
for h in $(seq 0 23); do
  run_gate env PIPELINE_NOW_HOUR="$h" PIPELINE_SCHEDULE_FILE="$F" bash "$GATE"
  case " 9 14 20 " in
...
run_gate env PIPELINE_NOW_HOUR=14 PIPELINE_SCHEDULE_FILE="$F3" bash "$GATE"
...
F4="$TMP/bad_hours.md"
printf '%s\n' '```pipeline-schedule' 'enabled: yes' 'hours: 9 noon 20' '```' > "$F4"
run_gate env PIPELINE_NOW_HOUR=9 PIPELINE_SCHEDULE_FILE="$F4" bash "$GATE"
[ "$VRC" -eq 1 ] && ok "a bad token poisons the list → skip (strict, fail-closed)" || bad "g7.rc" "exit=$VRC ($VOUT)"
# Out-of-range too.
F5="$TMP/oor_hours.md"
printf '%s\n' '```pipeline-schedule' 'enabled: yes' 'hours: 9 26' '```' > "$F5"
run_gate env PIPELINE_NOW_HOUR=9 PIPELINE_SCHEDULE_FILE="$F5" bash "$GATE"
...
F6="$TMP/boards.md"
printf '%s\n' '```pipeline-schedule' 'enabled: yes' 'hours: 14' 'boards: 3 4' '```' > "$F6"
run_gate env PIPELINE_NOW_HOUR=14 PIPELINE_SCHEDULE_FILE="$F6" bash "$GATE"
...
# No boards: → empty string (wrapper falls back to PIPELINE_ENABLED_BOARDS).
run_gate env PIPELINE_NOW_HOUR=14 PIPELINE_SCHEDULE_FILE="$F" bash "$GATE"
...
# setting, not a spend gate): it is dropped to empty + a stderr note, and the hour
# still RUNS. The wrapper then falls back to the PIPELINE_DRIVE_CAP code default.
echo "--- gate 9: cap: override surfaced; malformed cap runs (not fail-closed) (#642) ---"
F7="$TMP/cap.md"
printf '%s\n' '```pipeline-schedule' 'enabled: yes' 'hours: 14' 'cap: 3' '```' > "$F7"
run_gate env PIPELINE_NOW_HOUR=14 PIPELINE_SCHEDULE_FILE="$F7" bash "$GATE"
...
# No cap: → empty string (wrapper falls back to PIPELINE_DRIVE_CAP).
run_gate env PIPELINE_NOW_HOUR=14 PIPELINE_SCHEDULE_FILE="$F" bash "$GATE"
...
F8="$TMP/cap-bad.md"
printf '%s\n' '```pipeline-schedule' 'enabled: yes' 'hours: 14' 'cap: lots' '```' > "$F8"
run_gate env PIPELINE_NOW_HOUR=14 PIPELINE_SCHEDULE_FILE="$F8" bash "$GATE"
...
F9="$TMP/cap-zero.md"
printf '%s\n' '```pipeline-schedule' 'enabled: yes' 'hours: 14' 'cap: 0' '```' > "$F9"
run_gate env PIPELINE_NOW_HOUR=14 PIPELINE_SCHEDULE_FILE="$F9" bash "$GATE"
[ "$VRC" -eq 0 ] && [ "$(jq -r '.cap' <<<"$VOUT")" = "" ] && ok "cap: 0 (<1) dropped to empty, still runs" || bad "g9.zero" "rc=$VRC cap=$(jq -r '.cap' <<<"$VOUT")"

# ── GATE 10: PIPELINE_SCHEDULE_FILE default resolution — Controls/ then legacy
#    Context/ fallback (temperloop#226/#232, docs/config-precedence.md §
#    "operator controls"). UNLIKE gates 1-9 above (which all inject
#    PIPELINE_SCHEDULE_FILE explicitly and so never exercise the default
...
echo "--- gate 10: PIPELINE_SCHEDULE_FILE default — Controls/ then Context/ fallback ---"
KROOT="$TMP/kstore"
mkdir -p "$KROOT/Controls" "$KROOT/Context"

# 10a: Controls/ file present (Context/ absent) → the Controls/ path is used.
printf '%s\n' '```pipeline-schedule' 'enabled: yes' 'hours: 14' '```' > "$KROOT/Controls/foundation - pipeline schedule.md"
...
rm -f "$KROOT/Controls/foundation - pipeline schedule.md"

# 10b: Controls/ absent, legacy Context/ present → falls back and still fires.
printf '%s\n' '```pipeline-schedule' 'enabled: yes' 'hours: 14' '```' > "$KROOT/Context/foundation - pipeline schedule.md"
run_gate env PIPELINE_NOW_HOUR=14 KNOWLEDGE_STORE_ROOT="$KROOT" bash "$GATE"
...
printf '%s\n' '```pipeline-schedule' 'enabled: no' 'hours: 14' '```' > "$KROOT/Controls/foundation - pipeline schedule.md"
run_gate env PIPELINE_NOW_HOUR=14 KNOWLEDGE_STORE_ROOT="$KROOT" bash "$GATE"
[ "$VRC" -eq 1 ] && ok "Controls/ wins over Context/ when both exist (Controls/'s enabled:no honored)" || bad "g10c.rc" "exit=$VRC ($VOUT)"
rm -f "$KROOT/Controls/foundation - pipeline schedule.md" "$KROOT/Context/foundation - pipeline schedule.md"
...
OUT="$(env PIPELINE_NOW_HOUR=14 PIPELINE_NOW_DATE=2026-06-25 PIPELINE_NOW_TS=2026-06-25T14:00:00Z PIPELINE_SCHEDULE_FILE="$F" \
  PIPELINE_LOG_DIR="$LOGD" PIPELINE_NOTIFY_CMD="$NOTIFY" NOTIFY_SENTINEL="$SENT" \
...
grep -q "pipeline:" "$SENT" 2>/dev/null && ok "notify received the run summary" || bad "w1.notify-body" "$(cat "$SENT" 2>/dev/null)"
...
OUT2="$(env PIPELINE_NOW_HOUR=3 PIPELINE_NOW_DATE=2026-06-25 PIPELINE_NOW_TS=2026-06-25T03:00:00Z PIPELINE_SCHEDULE_FILE="$F" \
  PIPELINE_LOG_DIR="$LOGD2" PIPELINE_NOTIFY_CMD="$NOTIFY" NOTIFY_SENTINEL="$SENT2" \
...
OUT3="$(env PIPELINE_NOW_HOUR=14 PIPELINE_NOW_DATE=2026-06-25 PIPELINE_SCHEDULE_FILE="$F" \
  PIPELINE_LOG_DIR="$LOGD3" PIPELINE_NOTIFY_CMD="$NOTIFY" NOTIFY_SENTINEL="$SENT3" \
...
# tick board 4 (the override), not the default PIPELINE_ENABLED_BOARDS (=3).
FX4="$TMP/wfx4"; mkdir -p "$FX4/board-4"
printf '%s\n' '[{"number":701,"title":"foundation chore","labels":["Operational"]}]' > "$FX4/board-4/ready.json"
F4b="$TMP/sched_b4.md"
printf '%s\n' '```pipeline-schedule' 'enabled: yes' 'hours: 14' 'boards: 4' '```' > "$F4b"
LOGD4="$TMP/wlog4"
env PIPELINE_NOW_HOUR=14 PIPELINE_NOW_DATE=2026-06-25 PIPELINE_SCHEDULE_FILE="$F4b" \
  PIPELINE_ENABLED_BOARDS="3 4" PIPELINE_LOG_DIR="$LOGD4" PIPELINE_NOTIFY_CMD="true" \
...
env PIPELINE_NOW_HOUR=14 PIPELINE_NOW_DATE=2026-06-25 PIPELINE_SCHEDULE_FILE="$F" \
  PIPELINE_LOG_DIR="$TMP/sulog1" PIPELINE_NOTIFY_CMD="true" FOUNDATION="$R1" \
...
SU2OUT="$(env PIPELINE_NOW_HOUR=14 PIPELINE_NOW_DATE=2026-06-25 PIPELINE_SCHEDULE_FILE="$F" \
  PIPELINE_LOG_DIR="$TMP/sulog2" PIPELINE_NOTIFY_CMD="true" \
  PIPELINE_CRON_SELF_UPDATE=1 FOUNDATION="$R2" \
...
# With PIPELINE_CRON_SELF_UPDATED already set, the preamble must NOT fetch/reset/
...
SU3OUT="$(env PIPELINE_NOW_HOUR=14 PIPELINE_NOW_DATE=2026-06-25 PIPELINE_SCHEDULE_FILE="$F" \
  PIPELINE_LOG_DIR="$TMP/sulog3" PIPELINE_NOTIFY_CMD="true" \
  PIPELINE_CRON_SELF_UPDATE=1 PIPELINE_CRON_SELF_UPDATED=1 FOUNDATION="$R3" \
  bash "$CRON" --dry-run --fixture "$FXS" 2>/dev/null)"
...
SU4ERR="$TMP/su4.err"
SU4OUT="$(env PIPELINE_NOW_HOUR=14 PIPELINE_NOW_DATE=2026-06-25 PIPELINE_SCHEDULE_FILE="$F" \
  PIPELINE_LOG_DIR="$TMP/sulog4" PIPELINE_NOTIFY_CMD="true" \
  PIPELINE_CRON_SELF_UPDATE=1 FOUNDATION="$R4" \
...
# propagate, so PIPELINE_OPERATOR stays the placeholder and routing silently no-ops.
...
printf '%s\n' '# pipeline schedule' '' '```pipeline-schedule' 'enabled: yes' 'hours: 14' '```' > "$PSCHED"
...
env PIPELINE_NOW_HOUR=3 PIPELINE_SCHEDULE_FILE="$PSCHED" PIPELINE_LOG_DIR="$TMP/provlog1" \
  PIPELINE_NOTIFY_CMD="true" PIPELINE_OPERATOR_RESOLVE_BIN="$PRESOLVE" BUILD_CONFIG_LOCAL="$P1LOCAL" \
...
[ -f "$P1LOCAL" ] && grep -q 'export PIPELINE_OPERATOR="@provisioned-login"' "$P1LOCAL" \
...
env PIPELINE_NOW_HOUR=3 PIPELINE_SCHEDULE_FILE="$PSCHED" PIPELINE_LOG_DIR="$TMP/provlog2" \
  PIPELINE_NOTIFY_CMD="true" PIPELINE_OPERATOR_RESOLVE_BIN="$PRESOLVE_FAIL" BUILD_CONFIG_LOCAL="$P2LOCAL" \
...
env PIPELINE_NOW_HOUR=3 PIPELINE_SCHEDULE_FILE="$PSCHED" PIPELINE_LOG_DIR="$TMP/provlog3" \
  PIPELINE_NOTIFY_CMD="true" PIPELINE_OPERATOR="@realops" PIPELINE_OPERATOR_RESOLVE_BIN="$PRESOLVE" \
...
env PIPELINE_NOW_HOUR=3 PIPELINE_SCHEDULE_FILE="$PSCHED" PIPELINE_LOG_DIR="$TMP/provlog4" \
  PIPELINE_NOTIFY_CMD="true" PIPELINE_OPERATOR_RESOLVE_BIN="$PRESOLVE" BUILD_CONFIG_LOCAL="$P4LOCAL" \
...
# ── LAYER-5b drive step (#604) ────────────────────────────────────────────────
# Step 4 is OPT-IN (PIPELINE_DRIVE). A cron --dry-run passes --dry-run THROUGH to
# pipeline-drive.sh, so these stay offline — no claude is ever spawned. A marker
...
# ── WRAPPER 5: PIPELINE_DRIVE unset → Step 4 skipped (byte-for-byte 5a) ─────────
echo "--- wrapper 5: PIPELINE_DRIVE off → no drive step, stdout drive=off ---"
FX5="$TMP/wfx5"; mk_fixture "$FX5"
LOGD5="$TMP/wlog5"; MARK5="$TMP/mark5"
OUT5="$(env PIPELINE_NOW_HOUR=14 PIPELINE_NOW_DATE=2026-06-25 PIPELINE_SCHEDULE_FILE="$F" \
  PIPELINE_LOG_DIR="$LOGD5" PIPELINE_NOTIFY_CMD="true" \
  CLAUDE_BIN="$DRIVE_DOUBLE" DRIVE_MARK="$MARK5" \
  bash "$CRON" --dry-run --fixture "$FX5")"
[ "$(jq -r '.drive' <<<"$OUT5")" = "off" ] && ok "drive=off when PIPELINE_DRIVE unset" || bad "w5.drive" "got $(jq -r '.drive' <<<"$OUT5")"
[ ! -f "$MARK5" ] && ok "no claude spawned with the flag off" || bad "w5.spawn" "claude ran with PIPELINE_DRIVE off"
...
# ── WRAPPER 6: PIPELINE_DRIVE=1 + spike Ready → drive record, spike in safe[] ───
echo "--- wrapper 6: PIPELINE_DRIVE=1 (dry-run) → drive record, spike driven, no spawn ---"
...
OUT6="$(env PIPELINE_NOW_HOUR=14 PIPELINE_NOW_DATE=2026-06-25 PIPELINE_SCHEDULE_FILE="$F" \
  PIPELINE_LOG_DIR="$LOGD6" PIPELINE_NOTIFY_CMD="true" \
  PIPELINE_DRIVE=1 CLAUDE_BIN="$DRIVE_DOUBLE" DRIVE_MARK="$MARK6" \
...
# ── WRAPPER 7: PIPELINE_DRIVE=1 + code Ready → code left in merge[], not driven ─
echo "--- wrapper 7: PIPELINE_DRIVE=1 + code item → left for manual (merge[], status empty) ---"
FX7="$TMP/wfx7"; mk_fixture "$FX7"   # #101 is a plain Operational (code) item
LOGD7="$TMP/wlog7"
env PIPELINE_NOW_HOUR=14 PIPELINE_NOW_DATE=2026-06-25 PIPELINE_SCHEDULE_FILE="$F" \
  PIPELINE_LOG_DIR="$LOGD7" PIPELINE_NOTIFY_CMD="true" \
  PIPELINE_DRIVE=1 CLAUDE_BIN="$DRIVE_DOUBLE" DRIVE_MARK="$TMP/mark7" \
...
# The first live level-5b drive crashed: `pipeline-cron.sh: drive_dry[@]: unbound
...
# so the bug only fires once PIPELINE_DRIVE=1 actually drives — never in the
...
# must resolve the vault cap and thread it (export PIPELINE_DRIVE_CAP) to the tick,
...
printf '%s\n' '```pipeline-schedule' 'enabled: yes' 'hours: 14' 'cap: 3' '```' > "$F9c"
LOGD9="$TMP/wlog9"
env PIPELINE_NOW_HOUR=14 PIPELINE_NOW_DATE=2026-06-25 PIPELINE_SCHEDULE_FILE="$F9c" \
  PIPELINE_LOG_DIR="$LOGD9" PIPELINE_NOTIFY_CMD="true" \
...
printf '%s\n' '```pipeline-schedule' 'enabled: yes' 'hours: 14' '```' > "$F9d"
LOGD9b="$TMP/wlog9b"
env PIPELINE_NOW_HOUR=14 PIPELINE_NOW_DATE=2026-06-25 PIPELINE_SCHEDULE_FILE="$F9d" \
  PIPELINE_LOG_DIR="$LOGD9b" PIPELINE_NOTIFY_CMD="true" \
  bash "$CRON" --dry-run --fixture "$FX9" >/dev/null
...
env PIPELINE_NOW_HOUR=14 PIPELINE_NOW_DATE=2026-06-25 PIPELINE_SCHEDULE_FILE="$F" \
  PIPELINE_LOG_DIR="$LOGD10" PIPELINE_NOTIFY_CMD="true" \
...
OUT11="$(env PIPELINE_NOW_HOUR=14 PIPELINE_NOW_DATE=2026-06-25 PIPELINE_SCHEDULE_FILE="$F" \
  PIPELINE_GATE_BIN="$GATE_CRASH" PIPELINE_LOG_DIR="$LOGD11" PIPELINE_NOTIFY_CMD="true" \
...
env PIPELINE_NOW_HOUR=14 PIPELINE_NOW_DATE=2026-06-25 PIPELINE_SCHEDULE_FILE="$F" \
  PIPELINE_TICK_BIN="$TICK_CRASH" PIPELINE_LOG_DIR="$LOGD12" PIPELINE_NOTIFY_CMD="true" \
...
env PIPELINE_NOW_HOUR=14 PIPELINE_NOW_DATE=2026-06-25 PIPELINE_SCHEDULE_FILE="$F" \
  PIPELINE_DRIVE=1 PIPELINE_DRIVE_BIN="$DRIVE_CRASH" PIPELINE_LOG_DIR="$LOGD13" PIPELINE_NOTIFY_CMD="true" \
...
env PIPELINE_NOW_HOUR=14 PIPELINE_NOW_DATE=2026-06-25 PIPELINE_SCHEDULE_FILE="$F" \
  PIPELINE_CRON_SELF_UPDATE=1 FOUNDATION="$NOTAREPO" \
  PIPELINE_LOG_DIR="$LOGD14" PIPELINE_NOTIFY_CMD="true" \
...
# ╭──────────────────────────────────────────────────────────────────────────╮
# │ F#639 L0 — pipeline records are DUAL-WRITTEN to the git-archivable raw lake    │
# │ (meta/data/raw/pipeline-<YYYY-MM>.jsonl) alongside the home-dir log, so the    │
...
env PIPELINE_NOW_HOUR=14 PIPELINE_NOW_DATE=2026-06-25 PIPELINE_NOW_TS=2026-06-25T14:00:00Z PIPELINE_SCHEDULE_FILE="$F" \
  PIPELINE_LOG_DIR="$LOGD15" PIPELINE_RAW_DIR="$RAW15" PIPELINE_NOTIFY_CMD="true" \
  bash "$CRON" --dry-run --fixture "$FX15" >/dev/null
[ -f "$RAW15/pipeline-2026-06.jsonl" ] && ok "raw lake pipeline-2026-06.jsonl written (monthly rotation)" || bad "w15.file" "no $RAW15/pipeline-2026-06.jsonl ($(ls "$RAW15" 2>/dev/null))"
[ "$(jq -r '.event' "$RAW15/pipeline-2026-06.jsonl" | tail -1)" = "ran" ] && ok "the ran record is in the raw lake" || bad "w15.event" "got $(tail -1 "$RAW15/pipeline-2026-06.jsonl" 2>/dev/null)"
# The SAME record set is in both sinks (home-dir day log AND the raw lake).
[ "$(jq -c . "$LOGD15/2026-06-25.jsonl")" = "$(jq -c . "$RAW15/pipeline-2026-06.jsonl")" ] \
  && ok "home-dir log and raw lake carry the identical record set" || bad "w15.parity" "home=$(cat "$LOGD15/2026-06-25.jsonl") raw=$(cat "$RAW15/pipeline-2026-06.jsonl")"
[ "$(jq -r '.ts' "$RAW15/pipeline-2026-06.jsonl" | tail -1)" = "2026-06-25T14:00:00Z" ] && ok "raw record carries the pinned ts (#663 stamp preserved)" || bad "w15.ts" "got $(jq -r '.ts' "$RAW15/pipeline-2026-06.jsonl" | tail -1)"
...
ERR16="$TMP/w16.err"
env PIPELINE_NOW_HOUR=14 PIPELINE_NOW_DATE=2026-06-25 PIPELINE_SCHEDULE_FILE="$F" \
  PIPELINE_LOG_DIR="$LOGD16" PIPELINE_RAW_DIR="$BADRAW/sub" PIPELINE_NOTIFY_CMD="true" \
...
# │ via the PIPELINE_ISSUE_META_BIN test-double seam (mirrors GATE/TICK/DRIVE),   │
...
chmod +x "$IM_DOUBLE"
env PIPELINE_NOW_HOUR=14 PIPELINE_NOW_DATE=2026-06-25 PIPELINE_SCHEDULE_FILE="$F" \
  PIPELINE_LOG_DIR="$LOGD17" PIPELINE_NOTIFY_CMD="true" PIPELINE_ISSUE_META_BIN="$IM_DOUBLE" \
...
chmod +x "$IM_CRASH"
OUT18="$(env PIPELINE_NOW_HOUR=14 PIPELINE_NOW_DATE=2026-06-25 PIPELINE_SCHEDULE_FILE="$F" \
  PIPELINE_LOG_DIR="$LOGD18" PIPELINE_NOTIFY_CMD="true" PIPELINE_ISSUE_META_BIN="$IM_CRASH" \
...
OUT19="$(env PIPELINE_NOW_HOUR=14 PIPELINE_NOW_DATE=2026-06-25 PIPELINE_SCHEDULE_FILE="$F" \
  PIPELINE_LOG_DIR="$LOGD19" PIPELINE_NOTIFY_CMD="true" PIPELINE_ISSUE_META_BIN="$TMP/does-not-exist-im.sh" \
...
# tick stub for the non-dry-run wrapper tests below: pipeline-tick.sh normally
...
env PIPELINE_NOW_HOUR=14 PIPELINE_NOW_DATE=2026-06-25 PIPELINE_SCHEDULE_FILE="$F" \
  PIPELINE_LOG_DIR="$LOGD20" PIPELINE_NOTIFY_CMD="true" \
...
env PIPELINE_NOW_HOUR=14 PIPELINE_NOW_DATE=2026-06-25 PIPELINE_SCHEDULE_FILE="$F6" \
  PIPELINE_TICK_BIN="$TICK_NOOP" PIPELINE_LOG_DIR="$LOGD21" PIPELINE_NOTIFY_CMD="true" \
  PIPELINE_OPERATOR="@testops" \
...
OUT22="$(env PIPELINE_NOW_HOUR=14 PIPELINE_NOW_DATE=2026-06-25 PIPELINE_SCHEDULE_FILE="$F" \
  PIPELINE_TICK_BIN="$TICK_NOOP" PIPELINE_LOG_DIR="$LOGD22" PIPELINE_NOTIFY_CMD="true" \
  PIPELINE_OPERATOR="@testops" \
...
env PIPELINE_NOW_HOUR=14 PIPELINE_NOW_DATE=2026-06-25 PIPELINE_SCHEDULE_FILE="$F6" \
  PIPELINE_TICK_BIN="$TICK_NOOP" PIPELINE_LOG_DIR="$LOGD23" PIPELINE_NOTIFY_CMD="true" \
  PIPELINE_OPERATOR="@testops" \
...
env PIPELINE_NOW_HOUR=14 PIPELINE_NOW_DATE=2026-06-25 PIPELINE_SCHEDULE_FILE="$F" \
  PIPELINE_LOG_DIR="$LOGD23B" PIPELINE_NOTIFY_CMD="true" \
...
OUT24="$(env PIPELINE_NOW_HOUR=14 PIPELINE_NOW_DATE=2026-06-25 PIPELINE_SCHEDULE_FILE="$F6" \
  PIPELINE_TICK_BIN="$TICK_NOOP" PIPELINE_LOG_DIR="$LOGD24" PIPELINE_NOTIFY_CMD="true" \
  PIPELINE_OPERATOR="@testops" \
