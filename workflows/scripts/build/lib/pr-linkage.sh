...
# this file's own `closes_pattern` join key generates (temperloop#1910:
# workflows/scripts/config/join-keys.tsv's `closes_pattern` row,
# join-keys-lib.sh's `jk_closes_pattern` function — THE single home for this
# pattern; it is deliberately not restated here as a literal, so this
# comment does not re-drift from the registry the way the two pipeline
# copies below have already drifted from each other).
#
...
# JOIN-KEY REGISTRY (temperloop#1910): the closing-keyword pattern this file
# tests PR bodies against is generated by join-keys-lib.sh's
# `jk_closes_pattern` — never rebuilt inline here. Sourced relative to this
# file's own location so a caller need only source pr-linkage.sh itself.
_PR_LINKAGE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# shellcheck source=workflows/scripts/config/join-keys-lib.sh
source "$_PR_LINKAGE_DIR/../../config/join-keys-lib.sh"
...
open_pr_for_issue() {  # $1=repo  $2=issue
  ...
  local pat
  pat="$(jk_closes_pattern "$issue")" || return 0
