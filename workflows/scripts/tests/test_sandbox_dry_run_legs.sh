...
# Self-scoping gate (temperloop#361) — every leg below runs `temperloop init`
# / `eject` through a REAL bootstrapped dispatcher via
...
# satisfies. Runs BEFORE sourcing the sandbox lib so a composed-tree run exits
# 0 fast with zero setup, mirroring test_install_lifecycle.sh.
...
if _composed_reason="$(composed_tree_reason "$REPO_ROOT")"; then
  echo "SKIP: test_sandbox_dry_run_legs.sh — composed overlay tree detected ($_composed_reason)."
  echo "  This suite is scoped to a kernel-only checkout by design (temperloop#263):"
