...
#      TEMPERLOOP_* replacement + the v0.19.0 removal.
...
#      the post-v0.19.0 simulation seam): every legacy surface degrades
...
grep -q 'removed in v0.19.0' "$boot_err" || fail "1: the NOTEs must state the v0.19.0 removal"
...
grep -q 'removed in v0.19.0' "$closed_err" || fail "5a: the refusal must name the removal version"
[ ! -d "$CLOSED_HOME" ] || fail "5a: a refused bootstrap must install nothing"
pass "5a: window-closed bootstrap refuses legibly (names replacement + v0.19.0, installs nothing)"
...
grep -q 'removed in v0.19.0' "$init_closed" || fail "5b: init's refusal must name the removal version"
...
grep -q 'v0.19.0' "$probe_err" || fail "5c: the NOTE must name the removal version"
