...
- **No Seq ordering.** Retired by design (ADR 0004), not deferred — work
  ordering lives in epic dependency levels and milestones instead;
  `board_set_number` fails loud with a message naming the retirement rather
  than guessing a convention (no `fnd:seq:<n>` label encoding).
