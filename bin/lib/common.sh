#!/usr/bin/env bash
# kernel/bin/lib/common.sh — shared helpers + constants for the `foundation`
# CLI and its subcommands (foundation #765 Epic D, item
# cli-entrypoint-bootstrap / #849).
#
...
#
# Sourced, not executed — by the dispatcher (kernel/bin/foundation) directly,
...
# (see kernel/bin/foundation's header comment for that distinction):
...
# A PUBLIC surface for the sourcing scripts (kernel/bin/foundation today, a
...
FOUNDATION_CLI_HOME_DEFAULT="$HOME/.local/share/foundation-kernel"
# shellcheck disable=SC2034
FOUNDATION_CLI_BIN_DEFAULT="$HOME/.local/bin/foundation"
...
foundation_check_prereqs() {
  ...
  if ! command -v claude >/dev/null 2>&1; then
    echo "foundation: 'claude' (Claude Code CLI) not found on PATH." >&2
    echo "  Install: https://docs.claude.com/en/docs/claude-code/quickstart" >&2
    problems=$((problems + 1))
  fi

  if ! command -v gh >/dev/null 2>&1; then
    echo "foundation: 'gh' (GitHub CLI) not found on PATH." >&2
    echo "  Install: https://cli.github.com" >&2
    problems=$((problems + 1))
  elif ! gh auth status >/dev/null 2>&1; then
    echo "foundation: 'gh' is installed but not authenticated." >&2
