...
# removed in v0.19.0 (move the file: git has never tracked it — plain
...
if [ ! -f "$foundation_dir/baseline.jsonl" ] && [ -f "$repo_root/.foundation/baseline.jsonl" ]; then
  ...
  echo "baseline-snapshot: NOTE — appending to legacy ${foundation_dir#"$repo_root"/}/baseline.jsonl (dir renamed .temperloop/ in v0.15.0; legacy append removed in v0.19.0 — move the file)." >&2
fi
