...
case "$notice" in *v0.19.0*) ;; *) fail "shim NOTE must state the removal version v0.19.0 (got: $notice)" ;; esac
echo "PASS: shim prints one deprecation NOTE naming temperloop + removal version v0.19.0"

# Window-closed simulation (the post-v0.19.0 behavior, testable now): the
...
case "$out_c" in *v0.19.0*) ;; *) fail "window-closed shim failure must name v0.19.0 (got: $out_c)" ;; esac
echo "PASS: window-closed shim degrades legibly (exit $rc_c, names temperloop + v0.19.0)"
