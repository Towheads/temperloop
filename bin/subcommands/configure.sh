...
# settings (CONFIGURE_SETTINGS below) that writes its answers to exactly ONE
# file: the machine-conf file at precedence layer 3 of the six-layer ladder
...
# next time any machinery script sources build.config.sh, with zero further
...
# what this wizard (or a higher layer) actually resolved to.
#
# TWO MODES, chosen automatically (no flag needed) by whether `claude` is
# on PATH — this is a DEGRADATION, not a feature gate: every setting the
# wizard can set, it can set in EITHER mode; only the source of each
# setting's suggested value differs.
#
...
#   PLAIN PROMPTS (claude absent, or --no-ai): the same curated settings are
#     resolved via `--set NAME=VALUE` (if given), else an interactive
#     y/N-style per-setting prompt on a real tty, else the current/registry
...
#
# CONFIGURE_SETTINGS is a small, hardcoded starter set (PIPELINE_OPERATOR,
# PIPELINE_DRIVE_CONCURRENCY, BUILD_MERGE_GATE_WINDOW, BUILD_QUOTA_PAUSE_PCT) chosen
...
# already shows every registry setting for manual override via any layer; this
...
# no setting-registry.tsv row of its own.
#
# WRITE SAFETY: the machine-conf file is upserted line-by-line — an
# existing `: "${NAME:=...}"` line for a setting this run touches is REPLACED
# in place (awk, value passed via -v so no sed-metacharacter risk); every
# other line in the file (including any setting a PRIOR configure run or the
...
#
#   --set NAME=VALUE   Pre-answer one of CONFIGURE_SETTINGS (repeatable).
#                      Skips BOTH the AI suggestion and the interactive
#                      prompt for that setting — the primary non-interactive
...
#                      still-open setting, or accepting the non-interactive
...
if [ ! -f "$REGISTRY_LIB" ]; then
  echo "configure.sh: setting-registry-lib.sh not found at $REGISTRY_LIB (broken kernel checkout)" >&2
  exit 1
fi
# shellcheck source=../../workflows/scripts/config/setting-registry-lib.sh
source "$REGISTRY_LIB"

# Test-double seam (mirrors pipeline-drive.sh's CLAUDE_BIN convention — same
...
# summary order.
CONFIGURE_SETTINGS="PIPELINE_OPERATOR PIPELINE_DRIVE_CONCURRENCY BUILD_MERGE_GATE_WINDOW BUILD_QUOTA_PAUSE_PCT"
...
# kernel checkout) if the curated set names a setting the registry doesn't
# have — this should never happen absent a typo in CONFIGURE_SETTINGS.
_configure_registry_field() {
  local name="$1" field="$2" row
  row="$(setting_registry_rows | awk -F'\t' -v n="$name" '$1==n{print; exit}')"
  if [ -z "$row" ]; then
    echo "configure.sh: '$name' (in CONFIGURE_SETTINGS) has no setting-registry.tsv row — broken kernel checkout" >&2
    exit 1
  fi
  cut -f"$field" <<<"$row"
}
...
# as this setting's starting point: whatever the machine-conf file ALREADY
...
# ---------------------------------------------------------------------------
# The machine-conf target (layer 3's discovery path). Honors
# BUILD_CONFIG_MACHINE (already a setting-registry.tsv row) so this wizard
...
echo
echo "Target (layer 3 of the six-layer ladder, docs/config-precedence.md):"
echo "  $machine_conf_file"
echo

# ---------------------------------------------------------------------------
# Resolve each curated setting's seed (current-or-default) + registry
...
for name in $CONFIGURE_SETTINGS; do
...
# unresolved setting. Falls back to plain prompts for whatever it can't
...
if [ "$ai_available" -eq 1 ]; then
  ...
  for name in $unresolved; do
    ...
    seed="$(_configure_seed_value "$name" "$registry_default")"
    prompt_settings="$prompt_settings- $name (type: $type, current default: $seed) — $doc
"
  done

  prompt="$(cat <<PROMPT_EOF
You are suggesting starting values for a small set of local shell config
settings on the operator's own machine. You have NO tools (--tools ""); your
ONLY output is a single JSON object, applied by the calling script, never
by you.

Settings:
$prompt_settings
For each setting: if its current default already looks like a real, usable
...
{"SETTING_NAME": {"value": "<suggested value, as a plain string>", "why": "<one line>"}, ...}
PROMPT_EOF
)"
...
if [ -n "$unresolved" ]; then
  if [ "$ai_available" -eq 0 ]; then
    ...
    echo "-- Plain prompts (remaining settings) --"
  fi
...
# ---------------------------------------------------------------------------
# The write itself: per-setting upsert (replace an existing `: "${NAME:=...}"`
...
#!/usr/bin/env bash
#
# $machine_conf_file — temperloop machine-level config override (layer 3 of
# the six-layer precedence ladder, docs/config-precedence.md). Generated by
# \`temperloop configure\`; hand edits are preserved — configure only ever
# upserts the ONE line for a setting it manages, never anything else in this
...
echo "-- Written --"
echo "  $machine_conf_file ($n setting(s))"
echo
