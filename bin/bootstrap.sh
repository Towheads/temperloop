#!/bin/sh
# bootstrap.sh — one-line installer for the `foundation` CLI (foundation
...
#      public foundation-kernel repo into $FOUNDATION_HOME.
#   2. Symlinks $FOUNDATION_HOME/bin/foundation onto $FOUNDATION_BIN_DIR/foundation.
#   3. Prints a PATH reminder if $FOUNDATION_BIN_DIR isn't on it already.
#
# UNINSTALL: remove $FOUNDATION_BIN_DIR/foundation and $FOUNDATION_HOME —
# `foundation eject` documents removal of anything ELSE the CLI wrote to a
# target repo it was pointed at; this bootstrap's own footprint is exactly
# those two paths, nothing more.
#
...
if [ ! -f "$FOUNDATION_HOME/bin/foundation" ]; then
  echo "bootstrap: $FOUNDATION_HOME/bin/foundation not found after clone — the" >&2
...
echo "bootstrap: installed -> $FOUNDATION_BIN_DIR/foundation (-> $FOUNDATION_HOME/bin/foundation)"
...
echo "bootstrap: done. Next: foundation help"
