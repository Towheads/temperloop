...
| **CLI surface** | callers of `bin/foundation` and its subcommands (`init`, `eject`, `try`, `report`, `feedback`, `baseline-snapshot`, `configure`, `config`, `install`, `uninstall`, `update`) | `bin/foundation`, `bin/subcommands/*` |
